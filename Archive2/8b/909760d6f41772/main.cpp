...
struct AbstractItem
{
    ...
    void finalize()
    {
        ...
        for (auto& child : mChildren) { child->finalize(); }
        do_finalize();
    }

protected:
    void addChild(AbstractItem* child) { mChildren.emplace_back(child); }

private:
    void destroyChild(AbstractItem* child)
    {
        std::unique_ptr<AbstractItem> child_ptr(child);
        mChildren.erase(std::remove(mChildren.begin(), mChildren.end(), child_ptr), mChildren.end());
        child_ptr.release();
    }
    ...
    std::vector<std::unique_ptr<AbstractItem>> mChildren;
};
...
struct Item : AbstractItem
{
    ...
    {
        auto ptr = new T(dynamic_cast<Derived&>(*this), std::forward<Args>(args)...);
        addChild(ptr);
        return *ptr;
    }
...
private:
    ...
    void do_finalize() { std::cout << GetTypeName<Interface>() << "::do_finalize()" << std::endl; }
};
