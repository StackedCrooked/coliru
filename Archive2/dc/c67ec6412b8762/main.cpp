...
template < int offset_, typename... P >
...
//    }
    ...
    template <int k>
    void set( Tn<k> const& ref ) { cout << "set from X[y]" << endl; set<k>( (void*)&ref ); }
    
    template <int k>
    void set( V<k> const& ref ) { cout << "set from Vec<T,dim>" << endl; set<k>( (void*)&ref ); }

    template <int k>
    void set( array< EltType<k>, A<k>::dim > const& ref ) { cout << "set from std::array" << endl; set<k>( (void*)&ref ); }

    // Set & Get Attribute ELEMENT
...
int main()
{
    ...
    if((1)){
        using V = Attribute< int[2], int[3], int[4], int[5] >;
        COUT( V::A<0>::dim );
        COUT( V::A<1>::dim );
        COUT( V::A<2>::dim );
        COUT( V::A<3>::dim );
