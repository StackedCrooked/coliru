...
int main()
{
    struct Test1 { void f(int) const&; };
    struct Test2 { void f(int, ...) const volatile; };
    struct Test3 { void f(int) volatile&&; };
    
    print<get_std_function<decltype(&Test1::f)>::type>();
    print<get_std_function<decltype(&Test2::f)>::type>();
    print<get_std_function<decltype(&Test3::f)>::type>();
}