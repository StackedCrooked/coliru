...
int main() {

    {
        ...
        // older version of boost doesn't do initializer lists directly yet
        for (auto&& r : { VersionRecord
        ...
        // remove all with {4,38}
        auto group = table.get<mvKey>().equal_range(boost::make_tuple(4,38));
        table.erase(group.first, group.second);

        // now one of the ExpensiveCommonVersion should have been freed
        std::cout << "---\nQuerying for version id 8 again:\n";
        for (auto& record : boost::make_iterator_range(idx.equal_range(8)))
            std::cout << record << "\n";
    }
    std::cout << "Goodbye\n";
