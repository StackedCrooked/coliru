...
std::pair<double, double> compare(FuncOne&& one, FuncTwo&& two, std::size_t const iterations) {
	...
	return std::pair<double, double>{result.first / iterations, result.second / iterations};
}
...
int main() {
    ...
    for (std::string const& word: vec) {
		size_t lindex = -1;
		size_t bindex = -1;
		
		LinearSearch const ls{vec, word, lindex};
		BinarySearch const bs{vec, word, bindex};
		
		auto const timings = compare(ls, bs, 10000);
