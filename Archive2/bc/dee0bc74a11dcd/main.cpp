...
int main()
{
    const char* filename = "data.txt"; // the filename of the text you want to create / read from
    ...
    if( !fileOut ) // if file opening fails, then exit 
    {
    ...
    char strTemp[ 256 ]; // temporary variable, d2 maiistore ung ireread natin sa file (by line)
    // now read the datas line by line
    // kung gusto mo mag read by word, then use fileIn >> strTemp
    while( fileIn.getline( strTemp, 256 ) ) // loop until EOF or if file reading failed
    {
