...
#include <functional>
    ...
    bool compare( Range1&& range1, Range2&& range2) {
        typedef decltype(std::begin(range1)) ItemType;
        typedef typename std::iterator_traits<ItemType>::value_type ValueType;
        typedef typename std::equal_to<ValueType> PredType;
        PredType pred;
    	typedef range_capacity_compare<typename unqualified<PredType>::type> rcc_t;
		return compare( std::forward<Range1>( range1 ), std::forward<Range2>( range2 ), pred, rcc_t( ) );
	}
