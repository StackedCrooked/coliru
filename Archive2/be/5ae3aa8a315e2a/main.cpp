...
struct Event
{
    ...
    template <class Func>
    Event& operator+=(const Func& handler) {
        handlers.insert(handler);
        return *this;
    }
    
    template <class Func>
    Event& operator-=(function<void(Args...)> handler){
        handlers.erase(handler);
        return *this;
    }
private:
	set<function<void(Args...)>> handlers;
};
...
//    mon.Action([](bthing& t){return t.doSomething();});
   ...
   Event<int, int> ev2;
   ev2 += std::function<void(int, int)>([](int a, int b){cout << "a is " << a << " and  b is " << b << "\n";});
   ev2 += std::function<void(int, int)>([](int a, int b){cout << "2A IS " << a*2 << " AND 2B IS " << 2*b << "\n";});
   ev2(6,7);
