...
int main()
{
  ...
  C.report();
    ...
    size_t extend_size = 1e4;
    size_t iterations = 1e3;

    for( size_t x = 0; x < iterations; ++x ) {
        C.create( extend_size ); // 1 million more default constructed Elements
    }

    steady_clock::time_point end = steady_clock::now();

    std::cout << "\nExtending " << iterations << " times by " << extend_size << " and initializing the Elements took "
