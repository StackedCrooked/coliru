...
auto subrange = [] (auto subr) {
    return [=] (auto range) {
        auto a = subr.begin();
        auto b = range.begin();
        for (; a != subr.end(); ++a, ++b) {
            if (b == range.end()) {
                throw parse_error("expected '" + std::to_string(*a) + "' but got EOF");
            }
            if (*a != *b) {
                throw parse_error("expected '" + std::to_string(*a) + "' but got '" + std::to_string(*b) + "'");
            }
        }
        return std::make_pair(subr, range);
    };
};
...
auto sub = subrange(std::string("sub"));
