...
using DownTime  = boost::icl::interval_set<TimePoint>;
using Interval  = DownTime::interval_type;
using Records   = std::vector<DownTime>;
...
static DownTime generate_downtime(int j) {
    return boost::accumulate(
            boost::irange(0, 5),
            DownTime{},
            [j](DownTime accum, int i) { return accum + Interval::closed((i*10), ((i*10) + 5 - j)); }
        );
}
