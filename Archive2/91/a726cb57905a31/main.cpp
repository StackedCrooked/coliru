...
struct UnorderedSet {
    UnorderedSet(size_t& out, std::vector<int> const& stream): _out(out), _stream(stream) {}
...
int main() {
    ...
    for (size_t i = 0; i < SAMPLES; ++i) {
        size_t uset_result = 0;
        double uset = benchmark(UnorderedSet{uset_result, random_stream}, ITERATIONS);
        best_uset = std::min(best_uset, uset);
        std::cout << "Uset: " <<  uset_result << ": " << best_uset << "\n";
        /*
    ...
    std::cout << "Final: USET = " << best_uset << ", URES = " << best_ures << ", UNIQ = " << best_uniq << ", SORT = " << best_sort << "\n";
