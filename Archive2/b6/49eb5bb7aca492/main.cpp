...
int main()
{
    ...
    // Of course, string concatenation is probably the easiest string operation.
    cout << makesure << endl;
    ...
    cout << a << endl;
    // You can change any individual chars inside strings.
    a[0] = toupper(a[0]);
    a[12] = ',';
    cout << a << " hello." << endl << endl;
    
    // You can replace portions of a string with other text. Just indicate where to start, 
    //  how many chars to replace beginning there, and the text you want to use instead.
    ...
    //onelife.replace(0, onelife.size() - 1, otherlife);          // instead of this form
    onelife.replace(onelife.begin(), onelife.end(), otherlife);   // use this form instead
