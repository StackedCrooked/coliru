...
int main() {
    using NestedPack = Pack<long, Pack<int, Pack<double, char, long>>, char, long, short>;
    ...
    std::cout << std::boolalpha << std::is_same< decltype(a),
        Pack<char, long, short, long, Pack<int, Pack<double, char, long>>>
    >::value << std::endl;  // true

    TransformNestedPack<NestedPack, Rot, 2>::type b;
    std::cout << std::is_same< decltype(b),
        Pack<char, long, short, long, Pack<int, Pack<long, double, char>>>
    >::value << std::endl;  // true
    
    using NestedPack2 = Pack<int, char, Group<long, short, Wrap<std::string, short, long>, int>, double, long>;
    TD<TransformNestedPack<NestedPack2, Rot, 2>::type> c;
}