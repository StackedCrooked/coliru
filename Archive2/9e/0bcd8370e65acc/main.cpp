...
int main()
{
    ...
    // partially specialized template that makes an 
    // A,C pair when the 2nd arg is a pair<B,C> object
    PairMaker<StructA, std::pair<StructB, StructC>> p2;
    
    std::pair<StructA, StructB> pairMaker = 
        p1(a, b);
    // this will invoke the partial specialization version above
    std::pair<StructA, StructC> specialPairMaker = 
        p2(a, std::make_pair(b,c));
    ...
    auto comp = [](
        const std::pair<int, StructA>& lhs, 
        const std::pair<int, StructA>& rhs) -> bool { 
        return std::tie(lhs.second.mValue, lhs.first) <
            std::tie(rhs.second.mValue, rhs.first);
    };
    ...
    for (const auto& next : pairSet) {
        std::cout << "pair.first=" << next.first 
            << ", pair.second=" << next.second << std::endl;
    }
}
