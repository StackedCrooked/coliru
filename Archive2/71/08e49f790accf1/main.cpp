...
struct MLLEntry {
    ...
    // ordering operator - not sure if required?
    inline bool operator>=(const MLLEntry& rhs) const {
...
using IndexedFileInfo = multi_index_container<
    ...
    , indexed_by <
        hashed_unique < identity< MLLEntry > >
        // composite key ordered using nSetupCmd->nPutFileCmd->nCleanupCmd
        , hashed_non_unique<
            tag<composite>
            , composite_key <
                MLLEntry,
                BOOST_MULTI_INDEX_CONST_MEM_FUN(MLLEntry, int32_t, getSetupCmd),
                BOOST_MULTI_INDEX_CONST_MEM_FUN(MLLEntry, int32_t, getPutFileCmd),
                BOOST_MULTI_INDEX_CONST_MEM_FUN(MLLEntry, int32_t, getCleanupCmd)
            >
        >
        // sort by less<int> on mDirFilter
        , hashed_non_unique<
            tag<dirfilter>
            , BOOST_MULTI_INDEX_CONST_MEM_FUN(MLLEntry, int32_t, getDirFilter)
        >
    >
>;
...
int main()
{
    ...
    IndexedFileInfo c1;
    IndexedFileInfo::index<composite>::type& index3 = c1.get<composite>();
    IndexedFileInfo::index<dirfilter>::type& index4 = c1.get<dirfilter>();     
