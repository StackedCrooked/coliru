...
R reduce_impl(const std::array<T, N>&, F&&, R&& r, typename std::enable_if<(I==N),void>::type* =0)
{return std::forward<R>(r);}

template<size_t I, typename T, std::size_t N, class F, class R>
auto reduce_impl(const std::array<T, N>& a, F&& f, R&& r, typename std::enable_if<(I<N),void>::type* =0)
-> decltype(reduce_impl<I+1>(a, std::forward<F>(f), f(std::forward<R>(r), a[I])))
{
    auto&& p = f(std::forward<R>(r), a[I]);
    return reduce_impl<I+1>(a, std::forward<F>(f), std::move(p));
}

template<typename T, std::size_t N, class F>
auto reduce(const std::array<T, N>& a, F&& f) 
...
struct wierd_adder {
    template<int J>
    wierd_adder<I+J> operator+(wierd_adder<J>) const
    {return {};}
