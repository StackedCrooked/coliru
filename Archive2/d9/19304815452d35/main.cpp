...
struct aligned_union {
    ...
    template <typename ...R>
    static constexpr size_t max(size_t r0, R... r) {return ( r0 > max(r...) ? r0 : max(r...) );}
    
    static constexpr std::size_t alignment_value = max(alignof(Types)...);
    struct type {
        alignas(alignment_value) unsigned char _[max(Len, sizeof(Types)...)];
    };
};
...
public:
    ...
    T* get()
    {
        ...
        //return dynamic_cast<T*>(dynamic_cast<void*>(mem));
...
private:
    ...
    //typename std::aligned_union<usize, Types...>::type _storage;
