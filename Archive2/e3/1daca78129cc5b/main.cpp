...
// 2014 March 28  Aurthor : Mr Lam/ Yee Din(林貽典)  Email: yeedlam@yahoo.com
...
const double  SkyCircumference = 365.2575;   // original is 365.25 for Shoushi
const double  Sun_Y24Ref = 23.682;   //  23.71      23.71*23.7785/23.8070=23.68161612   23.7785 from page 787-729   23,8070 from page 787-717
...
{  
  ...
  printf("deg360\tSun_arc\t   b  \t  S_arc360\t       Sun_Lat360   TheoYR_Diff\t\tTheoSun_Lat\t    Sun_X    Center_Z  Center_Y\n");
  printf("\t\t\t    (dR=S_arc360-deg360)\t     (TheoYR_Diff-dR)\t(TheoSun_Lat-Sun_Lat360)\n");
  printf("===============================================================================================================================\n");
 
double  Center_Z24Ref = 53.3645;              // original is 56.0268 for Shoushi
double  deg360 = -1.0;
double  y;
int m,i;
for (m = 0; m - 1 < 90; m ++)
{
  ...
  double  Sun_Lat = Sun_Y + Sun_S_arcDiff;     //   以加黄赤道半背弦，即為黄赤道內外半弧背(Sun_Lat)
        //  Sun_Lat = sqrt(d*SY_b) + Sun_S_arcDiff/3.0;            // Supposed modified by 邢雲路  *SY_b) + Sun_S_arcDiff/3.0;
  ...
  printf("    %2.5f[%2.5f]\t%2.5f[%2.5f]\t%2.5f   %2.5f\t%2.5f\n", 
        TheoYR_Diff, TheoYR_Diff-(S_arc360-deg360), TheoSun_Lat, TheoSun_Lat-Sun_Lat360, Sun_X,Center_Z,Center_Y );
