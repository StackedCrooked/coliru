...
struct Object
{
    Object(PyObject* p)  { std::cout << "Converting PyObject :" << p->i << std::endl; }
};
...
template <typename T, typename std::enable_if<!std::is_same<typename std::decay<T>::type, PyObject*>{}, int>::type = 0>
T&& forward_convert(typename std::remove_reference<T>::type& a)
{
...
class Trampoline 
{
    ...
    using F3 = auto ( void* self, char a, Object )        -> float;
...
int main()
{
    ...
    float w  = trampoline.table.fp_3( base_as_pvoid, 'x', new PyObject{123} );   std::cout << w << std::endl; // expect: -3.14  (enabled and Final provides override, which throws!)
}
