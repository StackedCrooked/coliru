...
{    
    ...
    std::map<int32_t, std::shared_ptr<PriorityNode<PriorityLevel>>> priorityInfo;
    ...
    for (auto iter = gPriorities.begin(); iter != gPriorities.end();) {
        ...
        std::shared_ptr<PriorityNode<PriorityLevel>> currentNode, rootNode;
        size_t depth = 0;
        for (auto iter1 = ch_ptys.begin(); iter1 != ch_ptys.end();) {
            ...
            if (!currentNode) {
                currentNode = std::make_shared<PriorityNode<PriorityLevel>>(
                    concurrent, std::shared_ptr<PriorityNode<PriorityLevel>>(), depth++);
                if (!rootNode) {
                    rootNode = currentNode;
                }
            } else {
                currentNode->mNextNode = std::make_shared<PriorityNode<PriorityLevel>>(
                    concurrent, std::shared_ptr<PriorityNode<PriorityLevel>>(), depth++);
