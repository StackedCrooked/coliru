...
struct jones_household : public simulation_wrapper< woman, girl, cat >{
    int simulation(const woman& mrs_jones, const girl& mary, const cat& sniffles) {
         // mrs_jones and her daugther mary play with sniffles the cat
         std::cout << "int simulation(const woman& mrs_jones, const girl& mary, const cat& sniffles)\n";
         return 1;
    }
};

struct smith_household : public simulation_wrapper< man, dog >{
    int simulation(const man& mr_smith, const dog& fido) {
         // mr_smith and his dog fido go for a walk
         std::cout << "int simulation(const man& mr_smith, const dog& fido)\n";
         return 1;
    }
};
