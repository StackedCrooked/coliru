...
{
    template<typename Fun, typename... Args>
    holder(Fun&& f, Args&&... args)
        :   tic(high_resolution_clock::now()), 
            result(forward<Fun>(f)(forward<Args>(args)...))
    {
        auto toc = high_resolution_clock::now();
        elapsed = duration_cast<milliseconds>(toc - tic).count();
    }
...
template<typename Fun, typename... Args>
auto profile_this(Fun&& f, Args&&... args) 
    -> holder<decltype(forward<Fun>(f)(forward<Args>(args)...))>
{
    return {forward<Fun>(f), forward<Args>(args)...};
}
...
int main(int argc, char *argv[])
{
    ...
    auto res2 = prof([](auto e){ cout << "elapsed=" << e << endl; }, ff, 1, 2);
    
    auto hold = profile_this(ff, 1, 2);
    cout << hold.result << endl;
    cout << hold.elapsed << endl;
}