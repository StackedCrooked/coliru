...
public:
    ...
    template<class... Us>
    mystical_vector(Us&&... us)
        : mystical_vector(capacity_t{ sizeof...(Us) }, std::forward<Us>(us)...) {}
    
    template<class... Tuples>
    mystical_vector(std::piecewise_construct_t tag, Tuples&&... tuples)
        : mystical_vector(capacity_t{ sizeof...(Tuples) }, tag, std::forward<Tuples>(tuples)...) {}

    template<class... Us>
    mystical_vector(capacity_t capacity, Us&&... us)
        : _size(sizeof...(Us))
    ...
    template<class... Tuples>
    mystical_vector(capacity_t capacity, std::piecewise_construct_t, Tuples&&... tuples)
        : _size(sizeof...(Tuples))
        , _capacity(capacity.value)
        , _buf(make_buf())
    {
    ...
    mystical_vector(mystical_vector&&) = default;
    mystical_vector(mystical_vector const& other) = delete;
    
    ~mystical_vector(){ while (!empty()) pop_back(); }
    
    mystical_vector& operator=(mystical_vector other){ swap(other); return *this; }
    
    void swap(mystical_vector& other)
    {
...
int main()
{
    mystical_vector<brick> a1{1, 2, 3, 4, 5};
    mystical_vector<std::pair<brick, brick>> a2{
    ...
    mystical_vector<brick> a3{ capacity_t{10}, 1, 2, 3, 4, 5 };
    mystical_vector<std::pair<brick, brick>> a4{
        capacity_t{ 3 },
        std::piecewise_construct,
        std::make_tuple(std::piecewise_construct, std::make_tuple(1), std::make_tuple(2)),
        std::make_tuple(std::piecewise_construct, std::make_tuple(3), std::make_tuple(4))
    ...
    //mystical_vector<int> nope{1, 2, 3, 4, 5};
}