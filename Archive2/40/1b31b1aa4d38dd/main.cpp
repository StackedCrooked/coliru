...
int main()
{
  ...
  // Demonstrate framing the streambuf's input sequence manually.
  {
    ...
    std::cout << "framing " << bytes_transferred << " out of " <<
                 streambuf.size() << " bytes in the streambuf" << std::endl;
  ...
  // sequence.
  {
    ...
    std::istream input(&streambuf);
    int data = 0;
    input >> data; // Consumes "345" from input sequence.
    assert(data == 345);
    std::cout << "Extracted " << data << std::endl;
    assert(!input.eof());
    input.get(); // Consume "@" from input sequence.
  ...
  {
    read_until(client_socket, streambuf, '@');
