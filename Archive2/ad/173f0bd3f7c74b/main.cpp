...
fraction simplifier(fraction frac){
    ...
    while(abs(frac.numerator) >= frac.denominator) {  //reduce fractions such as 10/3 to 3 and 1/3.  
		if(frac.numerator > 0) frac.numerator = frac.numerator - frac.denominator;
		else frac.numerator = frac.numerator + frac.denominator; 
		frac.whole++;
	}
