...
struct LocalPool
{
    ...
    enum { LocalCapacity = 10 };

    LocalPool() : mImpl(std::allocate_shared<Impl<std::allocator<char>>>(std::allocator<char>(), std::allocator<int>()))
    {
    }

    template<typename Alloc, typename = DisableIf<IsRelated<LocalPool, Alloc>>>
    explicit LocalPool(Alloc alloc)
    {
...
int main()
{
    LocalPool pool(std::allocator<int>{});
