...
/**
 * Periodically runs %Callback on any scheduled device, after applying throttling logic.
 * 
...
protected:
	...
	bimultimap<time_t, device_id_t>::left_map::iterator getDueEndIterator()
	{
		...
		bimultimap<time_t, device_id_t>::left_map::iterator
			it   = schedule.left.begin(),
			end  = schedule.left.end();
		for ( ; it != end && it->first <= mt.tv_sec; ++it);
...
struct Manager
{
    ...
    void doFoo(const device_id_t&) {
        // executed periodically
    }
    
    void doBar(const device_id_t&) {
        // executed periodically
    }
...
private:
    ...
    struct FooCallback {
        void operator()(Manager* mgr, const device_id_t& device_id) {
            mgr->doFoo(device_id);
        }
    };
    
    struct FooDelay {
        time_t operator()(Manager*, const device_id_t&) {
    ...
    AdaptiveScheduler<Manager, FooCallback, FooDelay> fooScheduler;
    AdaptiveScheduler<Manager, BarCallback, BarDelay> barScheduler;
};
