...
struct BitSetUniquer {
    BitSetUniquer(size_t& out, std::vector<int> const& stream): _out(out), _stream(stream) {}
    
    void operator()() const {
        static size_t const BitsPerBucket = 64;
        
        std::deque<uint64_t> uniq;
        uniq.resize((1ul << 32) / BitsPerBucket);

        for (int e: _stream) { uniq[unsigned(e) / BitsPerBucket] |= 1 << (unsigned(e) % BitsPerBucket); }

        _out = 0;
        for (uint64_t e: uniq) { _out += __builtin_popcountl(e); }
    }
...
int main() {
    ...
    double best_bitset = std::numeric_limits<double>::max();
    ...
    for (size_t i = 0; i < SAMPLES; ++i) {
        ...
        /*
         * => execution expired
        size_t bitset_result = 0;
        double bitset = benchmark(BitSetUniquer{bitset_result, random_stream}, ITERATIONS);
        best_bitset = std::min(best_bitset, bitset);
        std::cout << "BitS: " << bitset_result << ": " << best_bitset << "\n";
        */
    }
    
    std::cout << "Final: URES = " << best_ures << ", UNIQ = " << best_uniq << ", SORT = " << best_sort << ", BITSET = " << best_bitset << "\n";
