...
template <typename T>
std::vector<T> make_sequence(size_t length)
{
	std::vector<T> seq(length);
	auto inc = []() { static T count{0}; return count++; };
	std::generate(seq.begin(), seq.end(), inc);
	return seq;
}
...
{
	std::vector<size_t> pos = make_sequence<size_t>(data.size());
