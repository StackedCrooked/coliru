...
auto awesome_tuple_cat( Tuplen&&... tn ) {
	return awesome_tuple_dog( idx<stuff<>, 0, 0, std::tuple_size<typename std::decay<Tuplen>::type>::value...>( ), std::make_tuple( std::forward<Tuplen>( tn )... ) ); // Get the size of each tuple, pass it through; must use ::type on idx because `TBin` and `TBinn` end up being a long `stuff` expression
}
