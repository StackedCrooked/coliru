...
struct tseitin : boost::static_visitor<void> {
    ...
    void operator()(const binop<op_and>  &b) const { proceed(0, b.oper1, b.oper2); }
    void operator()(const binop<op_impl> &b) const { proceed(2, b.oper1, b.oper2); }
    void operator()(const binop<op_or>   &b) const { proceed(1, b.oper1, b.oper2); }
    ...
    void proceed(int nop, const expr &l, const expr &r) const {
        int p1 = m + 1;
        int p2 = m + 2;
        m += 2;
        std::vector<int> tmp;
  ...
  private:
    template <typename T, typename U> void recurse(T const &v, U p) const {
        boost::apply_visitor(tseitin(p), v);
    }
};
    ...
    })
    {
        ...
        try
        {
            ...
            else
            {
                ...
                std::cout << "count : " << m << "\n";
                boost::apply_visitor(tseitin(0), result);
