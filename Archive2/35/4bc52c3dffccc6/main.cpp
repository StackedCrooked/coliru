...
int main()
{
    ...
    // Foo is now a hollow shell of it's former self
    Foo bar = std::move(foo);
    
    // Also avoiding a copy here as well
    std::vector<Foo> fooVector;
    
    fooVector.push_back(std::move(bar));
    
    // Both foo and bar are now dead and shouldn't be used. Their destructors
    // are esentially no-ops at this point which just remove anything on the stack
    
    // Whoops, I used him after he was moved. We correctly built class Foo to
    // handle the null case so it doesn't explode. If it didn't, however, we'd
    // probably set off a lot of class invariants. That's why it's a good idea
    // to always check those, to catch stuff like this.
    foo.PrintValues();
}
