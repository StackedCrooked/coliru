...
namespace ast {
    ...
    template <class OpTag> struct binary_op;
    struct usub_op;

    typedef binary_op<struct addTag> add_op;
    typedef binary_op<struct subTag> sub_op;
    typedef binary_op<struct mulTag> mul_op;
    typedef binary_op<struct divTag> div_op;
...
struct evaluator : boost::static_visitor<ast::arithmetic_type> {
    int operator()(ast::arithmetic_type value) const { return value; }
