...
private:
    ...
    struct Key {
        ...
        bool operator< (const Key& other) const { return tuple() <  other.tuple(); }
      private:
        std::tuple<void const* const&, std::type_index const&, uintmax_t const&> tuple() const { return std::tie(m_object, m_type, m_funcHash); }
    };
...
public:
    ...
    uintmax_t getHash(void (Class::*func)(Params...) const) {
        std::cout << "getHash(func): Class = " << typeid(Class).name() << ", func-type: " << typeid(func).name() << "\n";
