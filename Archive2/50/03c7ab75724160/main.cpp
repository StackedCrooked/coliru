...
namespace meta
{
    ...
    namespace detail
    {
        ...
        template <typename T> struct make_integer_sequence_<T, 1>
        {
         ...
         // Glue two sets of integer_sequence together
        template <typename I1, typename I2, typename I3>
        struct integer_range_cat;

        template <typename T, T... N1, T... N2, T... N3>
        struct integer_range_cat<   integer_sequence<T, N1...>,
                                    integer_sequence<T, N2...>,
                                    integer_sequence<T, N3...>>
        {
            using type = integer_sequence<T, N1..., N2...,N3...>;
        };
        template <typename T, std::size_t S, std::size_t N >
        struct make_integer_range_
        : integer_range_cat<     meta::eval<make_integer_range_<T, S, N / 2>>,
                                 meta::eval<make_integer_range_<T, S + N / 2 ,N / 2>>,
                                 meta::eval<make_integer_range_<T, S +N / 2 + N / 2 , N % 2>>>{

        };

        template <typename T, std::size_t S> struct make_integer_range_<T, S, 0>
        {
            using type = integer_sequence<T>;
        };

        template <typename T, std::size_t S> struct make_integer_range_<T, S, 1>
        {
    ...
    /// Generate \c index_sequence containing integer constants [S,...,E-1].
    /// \par Complexity
    /// \f$ O(log(N)) \f$.
    template <std::size_t S, std::size_t E>
    using make_index_range = eval<detail::make_integer_range_<std::size_t, S, (E>S)? E-S : 0 >>;

//    find<
//                                as_list<make_integer_sequence<std::size_t, E>> ,
//                                meta::size_t<S> >;
...
template<int N>
struct Fibonachi{
    static const int zahl = Fibonachi<N-2>::zahl + Fibonachi<N-1>::zahl ; 
};

template<>
struct Fibonachi<0>{
    static const int zahl = 0; 
};
template<>
struct Fibonachi<1>{
    static const int zahl = 1; 
};

int main()
{
 
   using fruchtTypen = meta::list< Banane, Orange, Erdbeere >;
    
   using KreuzkombinationsTypen   = meta::transform<
                                        fruchtTypen,
                                        meta::bind_front< meta::quote<meta::list> , Erbse >
                                    >;
    
   // KreuzkombinationsTypen = Map[ {Banane, Orange, Erdbeere} ,  Liste[ Erbse, arg1 , arg2 , arg3 ]  ]
    
   std::cout << "KreuzTypen: " << type<KreuzkombinationsTypen>() << std::endl;
   
   std::cout << "Fibonachi 20: " << Fibonachi<20>::zahl << std::endl;
