...
struct employee
{
  ...
  employee(int id_,std::string name_,int age_) : id(id_), bar1(name_), bar2(reverse_string(name_)), age(age_){}

  friend std::ostream& operator<<(std::ostream& os,const employee& e)
  {
    os<<e.id<<" "<<e.bar1.name<<" "<<e.bar2.name<<" "<<e.age<<std::endl;
    return os;
  }
};
...
typedef multi_index_container<
  ...
  indexed_by<
    ...
    ordered_non_unique<
      tag<name2>,BOOST_MULTI_INDEX_MEMBER(employee,employee::Bar,bar2)>,
    ordered_non_unique<
...
int main()
{
  ...
  std::cout<<"by name2"<<std::endl;
  print_out_by<name2>(es);
  std::cout<<std::endl;
