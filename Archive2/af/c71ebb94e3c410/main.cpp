...
using is_specialized = std::integral_constant<bool,
...
auto do_list(const std::vector<T_IN>& input, std::vector<T_OUT>& output)
    -> typename std::enable_if<!is_specialized<T_IN, T_OUT>::value>::type
{
...
int main()
{
    std::vector<int> vi_in, vi_out; vi_in.resize(1);
    std::vector<foo_in_type> vf_in; std::vector<foo_out_type> vf_out; vf_in.resize(1);
    std::vector<bar_in_type> vb_in; std::vector<baz_out_type> vb_out; vb_in.resize(1);
