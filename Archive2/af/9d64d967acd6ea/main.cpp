...
struct PriorityNode {
    ...
    */
    ...
    void schedule(const SLDBConfigParams& rParam) {
        // search mConcurrent for a duplicate
        auto iter = std::find_if(mConcurrent.cbegin(), mConcurrent.cend(), 
            [&rParam](const SLDBConfigParams& next){
                return next.getModuleName() == rParam.getModuleName();
            });
        if (iter != mConcurrent.cend()) {
            std::vector<SLDBConfigParams> concurrent = { rParam };
            if (!mNextNode) {
                mNextNode = std::make_shared <
                    PriorityNode> (mDepth+1);
            }
...
int main()
{
    ...
    for (const auto& taskRange : ranges) {
        ...
        for (auto iter = taskRange.first; iter != taskRange.second;){
            ...
            std::cout << std::endl;
#endif
            auto rootNode = std::make_shared <PriorityNode>();
            auto node = rootNode;
            for (auto iter1 = chParams.begin(); iter1 != chParams.end();) {
                ...
                node->mNextNode = std::make_shared <PriorityNode>(node->mDepth+1);
                node = node->mNextNode;
                iter1 = range.second;
            }
