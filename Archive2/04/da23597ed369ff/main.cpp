...
struct lambda_wrapper<Lambda, Ret(Args...)> : function_impl<Ret(Args...)>
{    
    lambda_wrapper(Lambda l) : lambda(l) {}
...
{
    Noisy n;
    auto lambda = [&n]() { return n.foo(); };    
    function<int()> f;
    lambda_wrapper<decltype(lambda), int()>* wrapper = new lambda_wrapper<decltype(lambda), int()>{lambda};
    f.impl = wrapper;
