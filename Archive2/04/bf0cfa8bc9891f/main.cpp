...
template<template<typename...> class F, typename... PLACEHOLDERS, typename... ARGS>
struct holder_eval_impl<holder<F, PLACEHOLDERS...>, ARGS...> : public F<ARGS...> {};
...
using holder_call = holder_eval<holder<first_of,_1,_2,_3,_4>, bool, float, char, int>;

int main ()
{
    static_assert(std::is_same<call, bool>{}, "Oops!");
	static_assert(std::is_same<alias_call, bool>{}, "Oops!");
	static_assert(std::is_same<holder_call, bool>{}, "Oops!");
}
