...
std::vector< std::pair<int,double> > read_filtered( std::istream& stm, int day, temp_info::datum col )
{
    ...
    while( std::getline( stm, line ) )
    {
        ...
        if( ti.day == day ) seq.emplace_back( ti.day, ti.values[col] ) ;
    }
...
int main()
{
    ...
    for( auto pair : read_filtered( file, 2, temp_info::POINT_FIVE_M ) )
        std::cout << "day " << pair.first << "   temperature (0.5m) " << pair.second << '\n' ;
}
