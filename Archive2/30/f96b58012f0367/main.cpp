...
public:
    ...
    static constexpr  unsigned int  offset      = Next::count; // bytecount_in;
    static constexpr  unsigned int  size_bytes  = sizeof(Tn);
    //static constexpr  unsigned int  total_bytes = bytecount_in+sizeof(Tn);

    static constexpr  unsigned int  count = size_bytes + Next::count; // + Next::remaining_bytes;

    static void setup(int index, int total_bytes, int byteoffsets[] )
    {
        byteoffsets[index] = total_bytes - count;
        ...
        cout << endl << "setup(" << index << ")" << endl;
        COUT( k );
        cout << "Type:" << typeid(Type).name() << endl;
        COUT( dim ); 
        COUT( size_bytes );
        COUT( byteoffsets[index] ); 
//        COUT( bytes_from_P0 ); 
        
        Next::setup(index+1, total_bytes, byteoffsets );
    }
};
...
private:
    ...
    static constexpr  unsigned int  total_bytes = AR<N-1>::count;
...
//    static  int* byteoffsets() {
//        static int b[ N ];
//        return b;
//    }
    static int byteoffsets[N];
    static void setup()
    {
        AR<N-1>::setup( 0, total_bytes, byteoffsets/*()*/ );
    }
};
...
{    
    ...
    using V = AttributeHandler< int[1], int[2], float[3] >;
