...
#define TRANSFORM(InSpaceP, OutSpaceP) template<typename T, typename InSpace> vec2<T, OutSpaceP> transform(vec2<T, InSpace> in, InSpace, OutSpaceP); template<typename T> vec2<T, OutSpaceP> transform(vec2<T, InSpaceP> in, InSpaceP, OutSpaceP)
...
namespace detail {
    ...
    // Specializations
    // Map -> Window
    TRANSFORM(MapSpace, WindowSpace) {
