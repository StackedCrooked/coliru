...
int main()
{
    ...
    for( int i = 0 ; i < 10 ; ++i )
    {
        auto big = random_number() ;
        while( big.size() < 1000 ) big.insert( big.end(), big.begin(), big.end() ) ;
        std::shuffle( big.begin(), big.end(), std::mt19937( std::time(nullptr) ) ) ;
        std::cout << next_palindrome(big).size()  << '\n' ;
    }
}
