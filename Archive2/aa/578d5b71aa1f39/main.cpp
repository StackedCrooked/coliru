...
// Partial specialization without trailing parameter pack
template <template <typename...> class ContainerType, typename... Args>
struct ConditionContainnerType<EContainnerType::EContainnerType_OR, ContainerType<Args...>>
{
    void foo() { std::cout << "no parameter-pack" << std::endl; }
};
...
int main()
{
    ...
    ConditionContainnerType<EContainnerType::EContainnerType_OR, std::vector<int>> x5; // OR and no parameter-pack
    x5.foo();
}
