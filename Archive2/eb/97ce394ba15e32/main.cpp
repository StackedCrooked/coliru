...
namespace ast
{
    ...
    manip::LocationInfoPrinter LocationInfo::printLoc() const { return manip::LocationInfoPrinter( *this ); }
...
struct ParseGrammar: public qi::grammar<Iterator, ast::SourceCode(), Skipper>
{
        ...
        annotate(first)
    {
        ...
        Identifier_     = as_string [ alpha >> *(alnum | char_("_")) ];
        Identifier      = (iter_pos >> Identifier_  >> iter_pos)[_val=_2,annotate(_val,_1,_3)];
        VarAssignment_  = KeywordVar > Identifier > '=' > int_ > SemiColon;
        VarAssignment   = (iter_pos >> VarAssignment_ >> iter_pos)[_val=_2,annotate(_val,_1,_3)];
        SourceCode_     = KeywordFunction >> Identifier >> '{' >> *VarAssignment >> '}';
        SourceCode      = (iter_pos >> SourceCode_ >> iter_pos)[_val=_2,annotate(_val,_1,_3)];
        
        //an alternative way to do this without needing the extra rules (ugh!)
        //Identifier      = (iter_pos >> as_string [ alpha >> *(alnum | char_("_")) ] >> iter_pos)[phx::at_c<0>(_val)=_2,annotate(_val,_1,_3)];
        //VarAssignment   = (iter_pos >> KeywordVar > Identifier > '=' > int_ > SemiColon >> iter_pos)[phx::at_c<0>(_val)=_2,phx::at_c<1>(_val)=_3,annotate(_val,_1,_4)];
        //SourceCode      = (iter_pos >> KeywordFunction >> Identifier >> '{' >> *VarAssignment >> '}' >> iter_pos)[phx::at_c<0>(_val)=_2, phx::at_c<1>(_val)=_3,annotate(_val,_1,_4)];
    ...
    qi::rule<Iterator, ast::SourceCode(),    Skipper> SourceCode,SourceCode_;
    qi::rule<Iterator, ast::VarAssignment(), Skipper> VarAssignment,VarAssignment_;
    qi::rule<Iterator, ast::Identifier()>             Identifier,Identifier_;
...
int main()
{
    ...
    if(ok && iter == last)
    {
        ...
        for(std::size_t i=0; i<program.assignments.size(); ++i)
        {
            const ast::VarAssignment& va=program.assignments[i];
