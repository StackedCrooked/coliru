...
struct PointGeneric {
	...
	// Copy Constructor
	PointGeneric(const PointGeneric<Tx, Ty>& other) : x(this), y(this), _x(std::move(other._x)), _y(std::move(other._y)) {};
...
template <typename Tx = float, typename Ty = Tx>
static PointGeneric<std::decay_t<Tx>, std::decay_t<Ty>> Point(Tx&& x, Ty&& y) {
