  ...
  private:
    struct Session : std::enable_shared_from_this<Session> {
        ...
        void Enqueue(int duration) {
            std::cout << std::setw(4) << mark() << " Enqueue for Number: "  << type <<  "(dur:"  << duration       <<  ")\n";
            queue_.push(duration);
            if (queue_.size() == 1)
                Wait();
        }
      ...
      private:
        void Close() {
            assert(!queue_.empty());
            std::cout << std::setw(4) << mark() << " Close for Number :  "  << type <<  "(dur:"  << queue_.front() <<  ") (depth " << queue_.size() << ")\n";
        ...
        void Wait() {
            if (!queue_.empty()) {
                std::cout << std::setw(4) << mark() << " Open for Number :   "  << type <<  "(dur:"  << queue_.front() <<  ") (depth " << queue_.size() << ")\n";
...
void Test::TimedOpen(int type, int duration) {
    ...
    session->strand_.post([=] { session->Enqueue(duration); });
}
