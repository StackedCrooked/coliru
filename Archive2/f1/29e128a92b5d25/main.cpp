...
inline std::size_t asm_log2(const std::size_t x) {
    if (x == 0) {
        return 0;
    }
    std::size_t y;
...
inline std::size_t gnu_asm_log2(const std::size_t x) {
    if (x == 0) {
        return 0;
    }
    return sizeof(std::size_t)*8-1 - __builtin_clzl(x);
}
...
int main() {
    ...
    */
    std::cout << "asm_log2:\t" << measure([&]{
        for (std::size_t i = 0; i < iterations; ++i) {
            results[2][i] = asm_log2(i);
        }
    }) << '\n';
    std::cout << "gnu_asm_log2:\t" << measure([&]{
        for (std::size_t i = 0; i < iterations; ++i) {
            results[2][i] = gnu_asm_log2(i);
        }
    }) << '\n';
    /*
