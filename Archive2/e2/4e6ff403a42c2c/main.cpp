...
int main () 
{
  ...
  { // Inner block's scope begins here.
    ...
    int x{42}; // Here in main()'s inner block's scope, another new (and totally different) 'x'
               //  variable declaration is OK, since we're protected here inside the inner
               //  block from having an 'x' name clash occur within our program 
               //  (comment out lines 19 & 31 to prove this for yourself).
    y = 77;    // Assign a new value into the (outer scope's) 'y' variable.
