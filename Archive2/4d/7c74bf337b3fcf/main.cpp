// Demonstrate the math function abs()
...
int main (){
    int sizediff{0}; // Always good for to immediately initialize a variable.
    
    //*** EDIT *** // What if was the FIRST vector that was shorter instead?
    // OH NOES!! :O
    ...
    if(s1 > s2) { //This test statement says that if the length of v1 is greater than the length of v2
        ...
        for(int idk = 0; idk < sizediff; idk++)
            //*** EDIT *** // Let's push_back on vector 2 which would be shorter in this case.
            v2.push_back(0); 
    }
    
    //*** EDIT *** // What if the vectors are equal in length?
    // So, let's add a second case test here too.
    ...
    if(s2 > s1) { // This test statement checks if the length of vector 2 is greater than the lengeth of vector 1.
        //*** EDIT *** // Let's swap the subtraction around, so we don't get a negative noomber.
        sizediff = s2 - s1; //myint will now be the difference between the second and first vector sizes
        
        //This loop states that it will add a new value in the smallest vector as many times
        //as there are more vectors in the largest.        
        for(int idk = 0; idk < sizediff; idk++)
            //*** EDIT *** // Let's push_back on vector 1 which would be shorter in this case.
            v1.push_back(0); 
    } 
