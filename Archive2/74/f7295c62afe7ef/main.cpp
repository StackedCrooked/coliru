...
void      PyObject_SetItem( PyObject* pyob, PyObject* key, PyObject* value ) { cout << "PyObject_SetItem( " << *pyob << " " << *key << " -> " << *value << " )" << endl; }
PyObject* PyObject_GetItem( PyObject* pyob, PyObject* key )                  { cout << "PyObject_GetItem( " << *pyob << " " << *key                     << " )" << endl;
...
public:
    ...
    public:
        ...
        Proxy&  operator= (const Proxy& rhs_ob) {
            cout << "Proxy::Proxy& operator= (const Proxy&)" << endl; 
            PyObject_SetItem( container, key, rhs_ob->ptr() );
        ...
        Object operator->() const { 
            cout << "Proxy::Object operator->()" << endl; 
    ...
    */
    const Object  operator[] (const Object& key)  const { 
        cout << "const Object Object::operator[]  (" << *p << "[" << *(key.p) << "] -> ...) CONST-SHORTCUT" << endl; 
        return { PyObject_GetItem( p, key.p ) }; 
    }

    Proxy operator[] (const Object& key) {
        cout << "Proxy Object::operator[]  (" << *p << "[" << *(key.p) << "] -> ...)" << endl; 
        return { p, key.p };
    }
    

    const Object* operator -> () const { 
        cout << "Object::const Object* operator -> " << endl; 
        return this; 
    }
