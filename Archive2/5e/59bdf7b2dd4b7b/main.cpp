...
namespace detail
{
	...
	underlying_type fromString( char const* str )
	{
		auto i = std::find_if(first, first + len, [str] (char const* arg) {return std::strcmp(arg, str) == 0;});

		if (i == first + len)
			throw std::invalid_argument("fromString(char const*)");
...
#define STRING_ENUM_IMPL(name, namespace_name, seq)                                                                                     \
      ...
      {                                                                                                                                 \
            ...
            constexpr auto info = EnumDefaults::detail::information(values, enum_size);                                                 \
                                                                                                                                        \
            enum_type fromString( char const* str )                                                                                     \
            {                                                                                                                           \
                  return static_cast<enum_type>(EnumDefaults::fromString<strings, enum_size, underlying_type, values>(str));            \
            }                                                                                                                           \
                                                                                                                                        \
...
int main()
{
	std::cout << MyEnumInfo::toString( MyEnumInfo::fromString("Enumerator1") ) << '\n';
	std::cout << MyEnumInfo::toString( MyEnumInfo::fromString("Enumerator2") );
}
