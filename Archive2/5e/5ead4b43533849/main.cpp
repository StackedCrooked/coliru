...
struct MyClass
{
    ...
    struct hasher {
        size_t operator()(MyClass const& o) const {
            auto s = boost::hash_value(o.name);
            boost::hash_combine(s, o.anInt1);
            return s;
        }
    };
...
int main()
{
    ...
    for(auto& b : boost::make_iterator_range(name_idx.equal_range("Mike")))
        std::cout << b << '\n';
...
void alternative_remove_mikes(Table& values) {
    ...
    auto it = std::partition(refs.begin(), refs.end(), [](boost::reference_wrapper<MyClass const> const& mc) { return "Mike" != mc.get().name; });
