...
string MessageToChiper()
{
    cout<<"Entrer le message a criper d'après le codage de césar"<<endl;
    string phrase;
    getline(cin, phrase);
    //cin.ignore('\n', 256);
    //std::cin >> var;
...
string MessageToDechiper()
{
    cout<<"Entrer le message a decriper d'après le codage de césar"<<endl;
    string phrase;
    getline(cin, phrase);
    return phrase;
}
...
string Dechiper(string phrase,int K)
{
    K = K % 26;
    K = -K;
    for(size_t i=0;i<phrase.length();i++)
...
int main()
{
    ...
    if(a==1)
    {
    //cout<<"phase codage"<<endl;
    Phrase=MessageToChiper();
    cle=KeyChiper();
    ...
    if(a==2)
    {
    //cout<<"phase decodage"<<endl;
    Phrase=MessageToDechiper();
    cle=KeyDechiper();
    Phrase=Dechiper(Phrase,cle);
    }
