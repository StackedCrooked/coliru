...
struct resource_loader
{
    ...
    explicit resource_loader( CALLABLE&& fn, ARGS&&... args ) :
...
int main()
{
    ...
    resource_loader loaders[] =
    {
        ...
        resource_loader( &sf::texture::load_from_file, t, "some_texture.dat", 0 ),

        // pass object by wrapped reference
        resource_loader( &sf::texture::load_from_memory, std::ref(t), buffer, 128, 7 ),

        // pass object by address
        resource_loader( &sf::font::load_from_file, pointer, "myfont.fnt" ),

        // pass parameter by wrapped fererence
        resource_loader( &sf::font::load_from_stream, std::ref(f), std::ref(file) ),

        // pass parameter by wrapped fererence to const
        resource_loader( &sf::font::load_from_font_cache, std::ref(f), std::cref(face_name), 12 ),

        // pass free function
        resource_loader( free_fun, std::addressof(t), "some_texture.dat",
                         std::ref(f), std::ref(file) ),

        // pass closure
        resource_loader( [&] { f.load_from_font_cache( face_name, 20 ) ; } ),

        // pass bind expression
        resource_loader( std::bind( &sf::texture::load_from_file, std::ref(t),
                                     "some_texture.dat", 0 ) ),

        // pass call wrapper
        resource_loader( std::function< void(sf::font&,std::istream&) >(&sf::font::load_from_stream),
                         std::ref(f), std::ref(file) )
