...
public:
    ...
    void increment() {
        assert(range_index<num_ranges*2-1 || current!=ranges[num_ranges*2-1]);
        ++current;
        if (range_index < num_ranges*2-1 && current==ranges[range_index]) {
            range_index+=2;
...
int main() {
    ...
    range = concatenate_ranges(vars0.begin(), vars0.end(), vars0.begin(), vars0.end(), vars0.begin(), vars0.end());
    for(auto iter=range.first; iter!=range.second; ++iter)
        std::cout << *iter << ' ';
    std::cout << '\n';
    
    range = concatenate_ranges(vars2.begin(), vars2.end(), vars2.begin()+1, vars2.end()-1, vars2.begin()+2, vars2.end()-2);
    for(auto iter=range.first; iter!=range.second; ++iter)
        std::cout << *iter << ' ';
    std::cout << '\n';
    
    range = concatenate_ranges(vars2.begin()+2, vars2.end()-2, vars2.begin()+1, vars2.end()-1, vars2.begin(), vars2.end());
    for(auto iter=range.first; iter!=range.second; ++iter)
        std::cout << *iter << ' ';
    std::cout << '\n';
