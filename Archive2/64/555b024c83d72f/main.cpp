...
std::istream& operator>>(std::istream& in, ship_type& type)
{
    std::string string_type;
    if (!(in >> string_type))
        return in;
    type = ship_type_from_string(string_type);
    return in;
}
std::ostream& operator<<(std::ostream& out, const ship_type& type)
