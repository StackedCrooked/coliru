...
template<typename CALLABLE, typename... Args>
struct function_traits
    : public function_traits<decltype(&CALLABLE::template operator()<Args...>)>
{
};
...
class std_function_convert
{
    ...
    public:
        using type = decltype(convert_to_std_function(typename std::make_integer_sequence<size_t, function_traits<CALLABLE>::arity>()));
};
...
int main()
{
    ...
    using traits2 = function_traits<PolyStruct, int(int)>;
    ...
    //std::function<int(int)> func = &PolyStruct::operator()<int>;
