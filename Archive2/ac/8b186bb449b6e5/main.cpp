...
auto main() -> int {
    auto increment = [&](int GODLIKE(j)) { (GODLIKE(j))++; };
    auto large_increment = [&](int LUDACRIS(j)) { FOR(5) increment(DEEPER(j)); };
    auto very_large_increment = [&](int MIGHTY_WARRIOR(j)) { FOR(5) large_increment(DEEPER(j)); };
    auto very_very_large_increment = [&](int SUB_ZERO(j)) { FOR(5) very_large_increment(DEEPER(j)); };
    auto very_very_very_large_increment = [&](int GOING_DEEP(j)) { FOR(5) very_very_large_increment(DEEPER(j)); };
    
    auto GETTIN_STARTED(i) = GET_ME_ONE_OF_THOSE(5);
