...
auto map_vec(F&& fnc, const std::vector<T>& source) -> decltype(fnc(std::declval<T>()), std::vector<T>())
{
    std::vector<T> dest;
...
template<typename T, typename F>
auto map_vec(F&& fnc, const std::vector<T>& source) -> decltype(fnc(std::declval<T>(), source), std::vector<T>())
{
    std::vector<T> dest;
    dest.reserve(source.size());
    for (const auto i : source)
