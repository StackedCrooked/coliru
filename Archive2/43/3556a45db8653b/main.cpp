...
/* 
   Author : Prashant Srivastava [_P0W!]
            Senior Engineer Prashant
            Government System - Displays
            Rockwell Collins, IDC.
            
   Demonstration for Embedding Python with C structure for Avionics Flight SW.
...
## Embeded Python External Script ####################################################################################
## embed.py

from ctypes import *
class SimulinkBus(Structure): ## C Struct Mappings
    _fields_ = [ ("a", c_int), ("b", c_char), ("c", c_float), ("d", c_double), ("e", c_char_p), ("arr", c_int*4) ]
  
def Logic( Signal ): ## Internal Routines
...
def main( *args,**kwargs ):
    ...
    ## Grab Bus passed from C code
    Signal = cast(c_char_p(kwargs['SimulinkBus']), POINTER(SimulinkBus )).contents
    print (Signal.a, Signal.b, Signal.c, Signal.d, Signal.e )
    
    for i in range(4):
        print ( Signal.arr[i] )
        Signal.arr[i] *= 2 ## Update the Bus elements
    Signal.c = Logic( Signal )
    
    ## Return Updated Bus to C
    return Signal
#######################################################################################################################

Note :
    In this online compiler, the python script embed.py is created using cat command on fly.
*/
...
int main(int argc, char *argv[])
{
    ...
    if (pModule != NULL) 
    {
        ...
        if (pFunc && PyCallable_Check(pFunc)) 
        {
            ...
            if (pValue != NULL) 
            {
                printf("\nFrom C code After Embeded Python Run :--- \n" );
