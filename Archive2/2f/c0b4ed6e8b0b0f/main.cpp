...
//unique_tuple<int,int,int> f= 3;


template <typename...Ts>
using pointer_list =meta::join<
                    meta::transform<
                        meta::apply_list< 
...
using a = pointer_list<int,double,int, float,unsigned, long>;

template <typename...Ts>
using pointer_list2 = meta::apply_list<
                                make_pointer, 
                                meta::list<Ts...>
                            >;

pointer_list2<int,double> ff= 4;

struct is_float_pointer{
    template<typename T>
    using apply = std::is_same<float*,T>;
};

using b = meta::find_if<a,is_float_pointer>;

struct remove_integral{
    template<typename T>
    using apply = typename std::conditional< std::is_integral< typename std::remove_pointer<T>::type >::value, meta::list<>, meta::list<T> >::type;
};

using c = meta::join< meta::transform<b, remove_integral> >;
c g = 3;
