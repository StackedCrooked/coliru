// NOTE
// ====
// - Purpose of this little experiment is to implement a task-queue using memory pools and allocators.
// - Its ingredients are:
//    + allocator constructs:
//        . packaged_task(allocator_arg_t, ...) constructor allows creating the task object using allocator (see TaskQueue::dispatch_impl method)
//        . std::allocate_shared<..> to allocate the Task::Impl in TaskQueue::Task (required for type-erasure)
//    + memory pools:
//        . FixedChunkingPool is based on Alexandrescu's Small Object Allocator
//        . FixedFreeListPool is a simple free-list (inspired by Poco::Pool)
//        . FlexiblePool wraps above two and selects pool based on requested block-size
//        . FlexiblePool uses a spin-lock for thread-safety (allocation and deallocation occur in different threads)
...
auto post_with_callback(T&& t, F&& f, CB&& cb) -> decltype(cb(f(t)), void())
{
    cb(f(std::forward<T>(t)));
}
    
template<typename T, typename F, typename CB>
auto post_with_callback(T&& t, F&& f, CB&& cb) -> decltype(cb(), void())
{
    f(std::forward<T>(t));
    cb();
}
...
struct TaskQueue
{
    ...
    void post(F&& f, Callback&& callback)
    {
        if (mQuit)
        {
            throw std::runtime_error("Quit");
        }
        post_impl(std::forward<F>(f), std::forward<Callback>(callback));
    }

    template<typename F>
    auto post(F&& f) -> std::future<decltype(f(std::declval<T&>()))>
    {
        if (mQuit)
        {
            throw std::runtime_error("Quit");
        ...
        return post_impl<R>(std::forward<F>(f));
    }
...
private:
    ...
    template<typename R, typename F>
    auto post_impl(F&& f) -> std::future<R>
    {
        // std::packaged_task allocation using allocator
        Allocator<char> alloc(mStorage);
        std::packaged_task<R(T&)> task(std::allocator_arg, alloc, std::forward<F>(f));
    ...
    template<typename F, typename Callback>
    void post_impl(F&& f, Callback&& cb)
    {
        // std::packaged_task allocation using allocator
        Allocator<char> alloc(mStorage);
        ...
        struct Helper
        {
            Helper(F&& f, Callback&& cb) : f(std::forward<F>(f)), cb(std::forward<Callback>(cb)) {}
            
            void operator()(T& t)
            {
                Detail::post_with_callback(t, f, cb);
            }
        ...
        // shared_ptr allocation also using allocator
        mQueue.push(Task(std::allocator_arg, alloc, Helper(std::forward<F>(f), std::forward<Callback>(cb))));
    }
    ...
    FlexiblePool mStorage;    tbb::concurrent_bounded_queue<Task> mQueue;
    T mObj;
    std::thread mThread;
};
...
int main()
{
    ...
    assert(task_queue.post([](int&){ return 1; }).get() == 1);
