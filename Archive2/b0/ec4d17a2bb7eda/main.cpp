...
template<typename TrivialInputIterator> // Orbit of transformation?
...
class NullTerminatedIterator
{
    const char *position;
public:
    explicit NullTerminatedIterator(const char *position)
        : position{position}
    {}
    ...
    friend bool empty(const NullTerminatedIterator &x)
    {
        return *x == 0;
    }
};
...
int main()
{
    ...
    for(auto x : make_range(NullTerminatedIterator{"Sentinel iteration"}))
        cout << x;
    cout << "!" << endl;
}
