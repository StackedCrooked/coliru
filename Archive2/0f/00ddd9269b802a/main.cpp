...
int main()
{
	...
	auto a7 = static_cast<int const * const>(icptr);
	cout << typeid(a7).name() << endl;

	auto a8 = reinterpret_cast<int const * const>(icptr);
	...
	cout << ( typeid(a10) == typeid(icptr) ) << endl;

	auto a12 = a10;
	cout << typeid(a12).name() << endl;
	cout << ( typeid(a12) == typeid(icptr) ) << endl;
};