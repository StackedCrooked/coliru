...
template <template <typename...> class T, typename D, std::size_t N, typename... Ts>
using template_with_params = typename template_with_params_c<(sizeof...(Ts) >= N), T, D, N, Ts...>::type;
...
int main()
{    
    template_with_params<B, void, 3, int>::type b{}; b.foo();
    
    static_assert(std::is_same< template_with_params<B, void, 3, int>::type, 
                                B<int, void, void>  >{}, "!");
                                
    static_assert(std::is_same< template_with_params<B, void, 1, int, float>::type, 
                                B<int>  >{}, "!");
}
