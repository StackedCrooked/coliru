...
using byte_int = unsigned int; // change to int in order to get proper debug output (chars are printed as chars)
...
struct Hdr_parser : qi::grammar<Iterator, qi::blank_type, Hdr()>
{
    ...
    qi::rule<Iterator, qi::blank_type, strong_bytebyte_int()> data_type_, byte_order_;
    qi::rule<Iterator, qi::blank_type, std::string()> interleave_;

    qi::uint_parser<byte_int,  2, 1, 1> byte_order_p; // 0 or 1
    qi::uint_parser<byte_int, 10, 1, 2> data_type_p;  // base 10, 2 digits
};
