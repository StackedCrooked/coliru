...
V construct(O &&... _o)
{
    ...
    using type_in_variant_constructible_from_args = typename boost::mpl::not_<typename boost::is_same<iter,typename boost::mpl::end<types>::type>::type>::type;
    static_assert(type_in_variant_constructible_from_args::value,"There isn't any type in the variant that fulfills the predicate");
    
    return construct_impl<V,iter>(type_in_variant_constructible_from_args(),std::forward<O>(_o)...);
}
