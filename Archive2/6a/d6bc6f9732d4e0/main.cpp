...
std::function<void(std::vector<boost::any> const&)> make_dispatcher(F f)
{
    ...
    return dispatcher_maker<args_type>{}.make(f); 
}
...
public:
    ...
    template<typename... Args>
    void fire(std::string const& event, Args&&... args)
    {
        auto rng = _callbacks.equal_range(event);
        for (auto it = rng.first; it != rng.second; ++it)
        {
            call(it->second, std::forward<Args>(args)...);
        }
    }
...
int main()
{
    ...
    events.listen("key", 
                  [] (int x, const char* c) 
                  { std::cout << "Got a " << x << " and a " << c << std::endl; });
    
    events.listen("key", 
                  [] (int x, const char* c, double d) 
                  { std::cout << "Got a " << x << " and a " << c << " as well as a " << d << std::endl; });
    ...
    events.fire("key", 42, "hi");
    
    std::cout << "Second firing:" << std::endl;
    
    events.fire("key", 42, "hi", 3.14);
}
