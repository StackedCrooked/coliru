...
{  
    {
        ...
        c.add("foo2", std::function<int(std::wstring)>(
                        std::bind(&A::foo2, &a, std::placeholders::_1)));
        
        c.call<int>("foo1");
        c.call<int>("foo2", std::wstring(L"Calling foo2"));
    }  
    {
        A a;
        AnyCaller c;
        c.add("foo1", std::function<int(A*)>(&A::foo1));
        c.add("foo2", std::function<int(A*, std::wstring)>(&A::foo2));
        
        c.call<int>("foo1", &a);
        c.call<int>("foo2", &a, std::wstring(L"Calling foo2"));
    }
}
