...
struct curry_t {
    ...
    {
		auto closure = [=, f=f](auto&&...ts)
		-> decltype(f(args..., std::declval<decltype(ts)>()...))
		{
			return f(args..., std::forward<decltype(ts)>(ts)...);
		};
		return curry_t< decltype(closure) >{ std::move(closure) };
	}
	template<class...Args,class=typename std::enable_if<is_invokable<F(Args...)>::value, int>::type>
	auto operator()(Args&&... args,...)const
	{
