...
int main () {
    std::unique_ptr<int*> arf( new int*(nullptr) );
    // You can have a ptrptr of another type with the static_cast built in!
    auto intptrptr = ptrptr<int*>( arf );
    int** double_indirection = intptrptr;
    //std::cout << **double_indirection << std::endl;
}