...
struct max_parser : qi::grammar<It, vector_and_max<T>(), Skipper> {
    ...
    max_parser(T const& start_value, ElementParser const& element_parser) : max_parser::base_type(start) {
        ...
        vector_with_max %= 
            ...
            >> *boost::proto::deep_copy(element_parser) 
                      [ running_max = maximum_(running_max, _1) ]
            >> attr(running_max)
            ;
  ...
  private:
    struct maximum_f {
        template <typename T1, typename...> struct result { typedef T type; }; // for old versions/compilers
        template <typename T1, typename T2> T operator()(T1 const& a, T2 const& b) const {
                return std::max(a, b);
            }
    };
