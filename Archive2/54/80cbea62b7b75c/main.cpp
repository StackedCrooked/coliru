...
std::pair<int, int> get_traffic_configuration(int mps)
{
  ...
  for (int i = 2; i < 1000; ++i)
  {
    ...
    int mpt = std::max(mps * 60 / tick_per_min, 1);
  ...
  auto p = min_element(begin(ts), end(ts), [=] (std::pair<int, int> a, std::pair<int, int> b) {
      return std::abs(mps * 60 - a.first * (int)(60000 / a.second)) < std::abs(mps * 60 - b.first * (int)(60000 / b.second));
  });
...
int main()
{
  auto f = [] (int i)
  {
    ...
    std::cout << "(" << v1.first << ", " << v1.second << ") :" << std::abs(i - v1.first * (int)(1000 / v1.second))  << std::endl;
  };
