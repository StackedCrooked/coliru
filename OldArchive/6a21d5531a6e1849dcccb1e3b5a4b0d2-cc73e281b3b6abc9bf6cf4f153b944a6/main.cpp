...
lua::table assign_to_lua(lua::state& lua, const person& src, lua::variant& dst) {
    lua::table t = dst.assign_new_table(); //I'm not sure I like this at all.  
    t.set("name", name); //t and dst refer to the same lua variable here
    t.set("age", age);
    t.set("print_age", person::get_age);
    return t;
}
