...
struct XML : Trace<XML<T>>
{
    XML(const T * = 0)
    {
        std::cout << "<proxy type=\"" << NAME(T) << "\">" << std::endl;
    }
...
template<typename T>
struct MyDecorator : XML<T>, MutexLock<T>
{
...
int main()
{
    ...
    Proxy<std::string, MyDecorator> string("abcd");
    
    string->resize(3);
    
    {
        auto scoped_proxy = string.operator->();
        std::string & s = *scoped_proxy;
        s = "abc";
    }
