...
struct parallel_for{
    parallel_for(int thread_count=std::thread::hardware_concurrency()) :threads(thread_count-1) {}
...
int main() {
    parallel_for{8}(0, 200, do_stuff); //note do_stuff takes the index as a param
}