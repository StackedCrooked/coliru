...
private:
    ...
    template<class... Us, class... Vs, class W, class X>
    static void match(std::tuple<predicate<W, X>, Us...> const& a, std::tuple<Vs...> const& b) {
        if (!std::get<0>(a).match(std::get<0>(b))) throw bad_match{"bad_match"};
        match(tuple_tail(a), tuple_tail(b));
    }

    template<class... Us, class... Vs>
    static void match(std::tuple<Us...> const& a, std::tuple<Vs...> const& b) {
        match(tuple_tail(a), tuple_tail(b));
    }
