...
int main()
{
    ...
    // runtime
    const auto str = "select col FROM table";
    const auto x2 = parse_select(str, strlen(str));
    std::cout << "SELECT " << std::string{std::get<0>(x2), std::get<1>(x2)} << " FROM " << std::string{std::get<2>(x2), std::get<3>(x2)} << std::endl;
}
