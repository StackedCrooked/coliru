...
# Default to the unprivileged runner uid so job artifacts are deletable by the webserver.
RUNNER_USER="${COLIRU_RUNNER_USER:-2001}"
...
RUNNER_MAX_FILESIZE_BYTES="$((RUNNER_MAX_FILESIZE_KB * 1024))"
...
docker run --rm \
    ...
    --ulimit "fsize=${RUNNER_MAX_FILESIZE_BYTES}:${RUNNER_MAX_FILESIZE_BYTES}" \
    ${EXTRA_ARGS} \
    --cap-drop ALL \
