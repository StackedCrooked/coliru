...
typename std::enable_if<HasResize< T1,typename T1::value_type>::value&&
(!HasReserve< T1,typename T1::value_type>::value),void>::type
inline reserveOrResize(T1& dst,T2&& src,typename T2::value_type newSize){
...
//enable if there  reserve function exists
template<class T1,class T2>
typename std::enable_if<HasReserve< T1,typename T1::value_type>::value,void>::type
inline reserveOrResize(T1& dst,T2&& src,typename T2::value_type newSize){
    static_assert(std::is_lvalue_reference<T1&>::value, "You must pass lvalue_refrence as first parameter");
    static_assert(std::is_rvalue_reference<T2&&>::value, "You must pass rvalue_refrence as second parameter");
    src.reserve(newSize);
