...
std::ostream& operator<<(std::ostream& os, const Graph& graph)
{
    ...
    typename boost::property_map<Graph, double VertexProperty::*>::type error_map = get(&VertexProperty::error, const_cast<Graph&>(graph));
...
{
  Graph g;
  boost::add_vertex(VertexProperty{1.0},g);
  boost::add_vertex(VertexProperty{3.0},g);
  std::cout << g <<std::endl;
}
