...
// 2014 April 29;  Aurthor : Mr Lam/ Yee Din(林貽典)  Email: yeedlam@yahoo.com
...
const double  Ref_dd = 0;  //d*d;     // Ref_dd = 上廉，用於授時曆時，Ref_dd = d*d; 以邢雲路新的多項弍推算, Ref_dd = 0
const double  Ref_ddd = 9*d*d*d;  // Ref_ddd = 益從方，用於授時曆時，Ref_ddd = d*d*d；以邢雲路新的多項弍推算, Ref_ddd = 9*d*d*d

const double  SkyCircumference = 365.2575;
const double  Sun_Y24Ref = 23.7785; // (23.78151; 23.80691);   23.7785 from page(787-729);  23.8070 from page(787-717) 
const double  Center_Z24Ref = 53.3645;
...
for (m = 0; m - 1 < 90; m ++)
{
  ...
  double  Center_Y_Off = Center_Y + 0.0016949*(91.31438 + 0.91539563 - Sun_arc);  //590.0825;  //suppose modified by 邢雲路
  //----------------------------------------------------------------------------------------------------------------
  ...
  double  SW = Sun_XRef/Center_Y;                     //            以赤道小弦(Center_Y)而一，為赤道大股(SW)，又為赤道半弧弦
       // SW/YZ = Center_S/Center_Y => SW = YZ * Center_S/Center_Y
       //                           => SW = Sun_X*(d/2)/Center_Y
  ...
  //________________________________________________________________________________
          Center_W = Center_W + 0.000528*Sun_arc;  // suppose modified by 邢雲路
  //--------------------------------------------------------------------------------
  ...
  //_________________________________________________________________________________
          S_arc = sqrt(d*WH_b) + S_arcDiff/3.0;  //suppose modified by 邢雲路
  //---------------------------------------------------------------------------------          
  double  S_arc360 = S_arc * 360/SkyCircumference; 
  printf("   %2.5f(%2.5f)", S_arc360, S_arc360 - deg360); //----------------------------------------------------------------------------
  double  SY_b=(d/2.0)-Center_Y_Off;//頁787-737(二至前後黄道距赤道內外度):  置半徑以減赤道小弦，餘為赤道二弦差，一云赤道矢(SY_b)
                                               // 置半徑減黄道矢，餘為黃赤道弦，一為立股，若相近曰句，
  double  Sun_Y = Center_X * Sun_Y24Ref/(d/2.0); // 以二至內外黄赤道半弧弦乘之，以半徑六十度八七五(d/2 = 58.42250)而一，得黃赤道小弧弦(Sun_Y)
  ...
  //___________________________________________________________________________________
          Sun_Lat = sqrt(d*SY_b) + Sun_S_arcDiff/3.0;     // Suppose modified by 邢雲路
  //-----------------------------------------------------------------------------------          
