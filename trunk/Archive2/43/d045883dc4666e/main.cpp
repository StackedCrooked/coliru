...
public:
    ...
    ~bi_vec() {_erase(first,last); deallocate(buffer, endbuf-buffer);}
...
private:
    ...
    iterator _erase(const_iterator f_, const_iterator l_) 
    {
        ...
        if (remainleft > remainright) {
            iterator newlast = std::rotate(f, l+1, last);
            while(last != newlast)
                destroy(--last);
            return f;
        } else {
            iterator newfirst = std::rotate(first, f, l);
...
template bi_vec<int>; //fully instantiated!  HOORAY!
