...
class Accountants
{
    ...
    public:
        ...
        // 
        template<typename L>
        Accountants subset(L &&lambda);
};

template<typename L>
Accountants Accountants::subset(L &&lambda)
{
    std::vector<Accountant> v;
    std::copy(std::find_if(m_list.begin(), m_list.end(), lambda), m_list.end(), std::back_inserter(v));
    
    return Accountants(v);
}
