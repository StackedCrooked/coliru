...
template<class T>
using get_variant_t = boost::optional<invoke_t<get_variant, gen_seq_for<T>, T>>;

template<class T>
get_variant_t<T&&> get(std::size_t i, T&& t, int_<0>)
{
    if (i == 0)
        return get_variant_t<T&&>{make_tagged<0>(std::get<0>(std::forward<T>(t)))};
    return boost::none;
}
...
{
    if (i == I)
        return get_variant_t<T&&>{make_tagged<I>(std::get<I>(std::forward<T>(t)))};
