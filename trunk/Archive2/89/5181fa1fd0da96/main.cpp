...
struct PriorityNode {
    ...
    explicit PriorityNode(
        const std::vector<Param>& rConcurrent,
        /*const std::shared_ptr<PriorityNode<Param>>& rNext =
        std::shared_ptr<PriorityNode<Param>>(),*/
        const size_t& rDepth = 0)
        : mConcurrent(rConcurrent)
        , mRunningPeerCount(-1)
        , mNextNode(/*rNext*/nullptr)
        , mDepth(rDepth)
        , mDone(false)
    {}
     ...
     */
    friend std::ostream& operator<<(
        std::ostream& os, const PriorityNode<Param>& rhs) {
...
int main()
{
    ...
    // 
    // module[nic1], ip[192.168.1.1], nic[192.168.1.1], prty[6A], pn[7038231-1902], dr/xr[envmoddir._dr]
    // module[nic1], ip[192.168.1.1], nic[192.168.1.1], prty[6A], pn[7038231-1902], dr/xr[INAVDATAHI_16M._xr]
    // module[nic1], ip[192.168.1.1], nic[192.168.1.1], prty[6A], pn[7038231-1902], dr/xr[jeppall._dr]
    // module[nic1], ip[192.168.1.1], nic[192.168.1.1], prty[6A], pn[7038231-1902], dr/xr[tfsdir._dr]
    // module[nic1], ip[192.168.1.1], nic[192.168.1.1], prty[6A], pn[7038231-1902], dr/xr[threatdir._dr]
    // module[aiopa1], ip[192.168.1.18], nic[192.168.1.1], prty[6B2], pn[7038231-1902], dr/xr[envmoddir._dr]
    // module[aiopa1], ip[192.168.1.18], nic[192.168.1.1], prty[6B2], pn[7038231-1902], dr/xr[INAVDATAHI_16M._xr]
    // module[aiopa1], ip[192.168.1.18], nic[192.168.1.1], prty[6B2], pn[7038231-1902], dr/xr[jeppall._dr]
    // module[aiopa1], ip[192.168.1.18], nic[192.168.1.1], prty[6B2], pn[7038231-1902], dr/xr[tfsdir._dr]
    // module[aiopa1], ip[192.168.1.18], nic[192.168.1.1], prty[6B2], pn[7038231-1902], dr/xr[threatdir._dr]
    // module[proc1], ip[192.168.193.1], nic[192.168.1.1], prty[6B1], pn[7038231-1902], dr/xr[envmoddir._dr]
    // module[proc1], ip[192.168.193.1], nic[192.168.1.1], prty[6B1], pn[7038231-1902], dr/xr[INAVDATAHI_16M._xr]
    // module[proc1], ip[192.168.193.1], nic[192.168.1.1], prty[6B1], pn[7038231-1902], dr/xr[jeppall._dr]
    // module[proc1], ip[192.168.193.1], nic[192.168.1.1], prty[6B1], pn[7038231-1902], dr/xr[tfsdir._dr]
    // module[proc1], ip[192.168.193.1], nic[192.168.1.1], prty[6B1], pn[7038231-1902], dr/xr[threatdir._dr]
    // module[agm1], ip[192.168.13.20], nic[192.168.13.1], prty[3B1], pn[7038231-1902], dr/xr[envmoddir._dr]
    // module[agm1], ip[192.168.13.20], nic[192.168.13.1], prty[3B1], pn[7038231-1902], dr/xr[INAVDATAHI_16M._xr]
    // module[agm1], ip[192.168.13.20], nic[192.168.13.1], prty[3B1], pn[7038231-1902], dr/xr[jeppall._dr]
    // module[agm1], ip[192.168.13.20], nic[192.168.13.1], prty[3B1], pn[7038231-1902], dr/xr[tfsdir._dr]
    // module[agm1], ip[192.168.13.20], nic[192.168.13.1], prty[3B1], pn[7038231-1902], dr/xr[threatdir._dr]
    ...
    for (const auto& taskRange : ranges) {
        ...
        for (auto iter = taskRange.first; iter != taskRange.second;){
            ...
            size_t depth = 0;
            std::shared_ptr<PriorityNode<SLDBConfigParams>> currentNode, rootNode;
            for (auto iter1 = chParams.begin(); iter1 != chParams.end();) {
                ...
                // remember the adjacent parent 
                std::shared_ptr<PriorityNode<SLDBConfigParams>> adjacentParent = nullptr;
                while (defIter != range.second) {
                    ...
                    if (tempIter != range.second) {
                        ...
                        defIter = tempIter + 1;
                        adjacentParent = currentNode;
                        // build the tree here
                        if (!currentNode) {
                            currentNode = std::make_shared <
                            ...
                            currentNode->mNextNode = std::make_shared <
                                PriorityNode <SLDBConfigParams >> (
                                concurrent, depth++);
                        ...
                        if (adjacentParent) {
                            std::copy(defIter, range.second,
                                std::back_inserter(adjacentParent->mConcurrent));
                        } else {
                            std::copy(defIter, range.second,
                                std::back_inserter(concurrent));
                            // build the tree here
