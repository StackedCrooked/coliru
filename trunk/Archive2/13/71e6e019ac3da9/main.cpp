...
//std::for_each(b0.begin(), b0.end(), PrintIt); std::cout<<"********\n";
    ...
    Score[1] = b0.size()- Score[0];

//std::for_each(b0.begin(), b0.end(), PrintIt); std::cout<<"********\n";

    std::for_each(b0.begin(), b0.end(), ListMoves); //only adds moves if not listed already 
    for (uint x = Score[0]; x<(b0.size()); x++)
        for (int y = 0; y<b0.at(x).move; y++) {
            b0.push_back(Move(b0.at(x),y));
        }
    Score[2] = b0.size() - Score[1] - Score[0];
        
//std::for_each(b0.begin(), b0.end(), PrintIt); std::cout<<"********\n";

    std::for_each(b0.begin(), b0.end(), ListMoves); //only adds moves if not listed already 
    for (uint x = Score[1]; x<(b0.size()); x++)
        for (int y = 0; y<b0.at(x).move; y++) {
            b0.push_back(Move(b0.at(x),y));
        }
    Score[3] = b0.size() - Score[2] - Score[1] - Score[0];

//std::for_each(b0.begin(), b0.end(), PrintIt); std::cout<<"********\n";

    std::for_each(b0.begin(), b0.end(), ListMoves); //only adds moves if not listed already 
    for (uint x = Score[2]; x<(b0.size()); x++)
        for (int y = 0; y<b0.at(x).move; y++) {
            b0.push_back(Move(b0.at(x),y));
        }
    Score[4] = b0.size() - Score[3] - Score[2] - Score[1] - Score[0];

//std::for_each(b0.begin(), b0.end(), PrintIt); std::cout<<"********\n";

    std::for_each(b0.begin(), b0.end(), ListMoves); //only adds moves if not listed already 
    for (uint x = Score[3]; x<(b0.size()); x++)
        for (int y = 0; y<b0.at(x).move; y++) {
            b0.push_back(Move(b0.at(x),y));
        }
    Score[5] = b0.size() - Score[4] - Score[3] - Score[2] - Score[1] - Score[0];

//std::for_each(b0.begin(), b0.end(), PrintIt); std::cout<<"********\n";

    std::for_each(b0.begin(), b0.end(), ListMoves); //only adds moves if not listed already 
    for (uint x = Score[4]; x<(b0.size()); x++)
        for (int y = 0; y<b0.at(x).move; y++) {
            b0.push_back(Move(b0.at(x),y));
        }
    Score[6] = b0.size() - Score[5] - Score[4] - Score[3] - Score[2] - Score[1] - Score[0];

//std::for_each(b0.begin(), b0.end(), PrintIt); std::cout<<"********\n";

    std::for_each(b0.begin(), b0.end(), ListMoves); //only adds moves if not listed already 
    for (uint x = Score[5]; x<(b0.size()); x++)
        for (int y = 0; y<b0.at(x).move; y++) {
            b0.push_back(Move(b0.at(x),y));
        }
    Score[7] = b0.size() - Score[6] - Score[5] - Score[4] - Score[3] - Score[2] - Score[1] - Score[0];

//std::for_each(b0.begin(), b0.end(), PrintIt); std::cout<<"********\n";

    std::for_each(b0.begin(), b0.end(), ListMoves); //only adds moves if not listed already 
    for (uint x = Score[6]; x<(b0.size()); x++)
        for (int y = 0; y<b0.at(x).move; y++) {
            b0.push_back(Move(b0.at(x),y));
        }
    Score[8] = b0.size() - Score[7] - Score[6] - Score[5] - Score[4] - Score[3] - Score[2] - Score[1] - Score[0];

//std::for_each(b0.begin(), b0.end(), PrintIt); std::cout<<"********\n";

    std::for_each(b0.begin(), b0.end(), ListMoves); //only adds moves if not listed already 
    for (uint x = Score[7]; x<(b0.size()); x++)
        for (int y = 0; y<b0.at(x).move; y++) {
            b0.push_back(Move(b0.at(x),y));
        }
    Score[9] = b0.size() - Score[8] -Score[7] - Score[6] - Score[5] - Score[4] - Score[3] - Score[2] - Score[1] - Score[0];

//std::for_each(b0.begin(), b0.end(), PrintIt); std::cout<<"********\n";

    std::for_each(b0.begin(), b0.end(), ListMoves); //only adds moves if not listed already 
    for (uint x = Score[8]; x<(b0.size()); x++)
        for (int y = 0; y<b0.at(x).move; y++) {
            b0.push_back(Move(b0.at(x),y));
        }
    Score[10] = b0.size() - Score[9] - Score[8] -Score[7] - Score[6] - Score[5] - Score[4] - Score[3] - Score[2] - Score[1] - Score[0];

//std::for_each(b0.begin(), b0.end(), PrintIt); std::cout<<"********\n";

    std::for_each(b0.begin(), b0.end(), ListMoves); //only adds moves if not listed already 
    for (uint x = Score[9]; x<(b0.size()); x++)
        for (int y = 0; y<b0.at(x).move; y++) {
            b0.push_back(Move(b0.at(x),y));
        }
    Score[11] = b0.size() - Score[10] - Score[9] - Score[8] -Score[7] - Score[6] - Score[5] - Score[4] - Score[3] - Score[2] - Score[1] - Score[0];

//std::for_each(b0.begin(), b0.end(), PrintIt); std::cout<<"********\n";

    std::for_each(b0.begin(), b0.end(), ListMoves); //only adds moves if not listed already 
    for (uint x = Score[10]; x<(b0.size()); x++)
        for (int y = 0; y<b0.at(x).move; y++) {
            b0.push_back(Move(b0.at(x),y));
        }
    Score[12] = b0.size() - Score[11] - Score[10] - Score[9] - Score[8] -Score[7] - Score[6] - Score[5] - Score[4] - Score[3] - Score[2] - Score[1] - Score[0];

//std::for_each(b0.begin(), b0.end(), PrintIt); std::cout<<"********\n";

    std::for_each(b0.begin(), b0.end(), ListMoves); //only adds moves if not listed already 
    for (uint x = Score[11]; x<(b0.size()); x++)
        for (int y = 0; y<b0.at(x).move; y++) {
            b0.push_back(Move(b0.at(x),y));
        }
    Score[13] = b0.size() - Score[12] - Score[11] -Score[10] - Score[9] - Score[8] -Score[7] - Score[6] - Score[5] - Score[4] - Score[3] - Score[2] - Score[1] - Score[0];
...
//    }
    ...
    for (int x = 0; x<15; x++) {std::cout << "Number of Level: "<<x<<"="<<Score[x]<<" for # of records:"<< b0.size() <<"\n";}
