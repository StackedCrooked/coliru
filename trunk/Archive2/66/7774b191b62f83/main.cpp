...
struct SegmentPool
{
    SegmentPool(std::size_t inSegmentSize) : mSegmentSize(inSegmentSize) {  std::cout << "SEGMENTSIZE=" << inSegmentSize << std::endl; }
...
struct MemoryPool
{
    ...
    {
        return getPool(length).allocate();
    }

    void deallocate(void* data, std::size_t length)
    {
        return getPool(length).deallocate(data);
    }

private:
    SegmentPool& getPool(std::size_t length)
    {
        auto it = mPools.find(length);
        if (it == mPools.end())
        {
            return *mPools.emplace(length, std::make_unique<SegmentPool>(length)).first->second;
        }
        return *it->second;
    }
