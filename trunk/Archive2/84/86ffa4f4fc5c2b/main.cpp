...
double dot_double2(int n, double *a, double *b ) {
	...
	int i;
	for(i=0; i< (n & (-2)); i+=2) {
        double2 a2 = *(double2*)(&a[i]);
        double2 b2 = *(double2*)(&b[i]);
	    sum2 += a2*b2;
	}
...
double dot_double2_unroll2(int n, double *a, double *b ) {
	...
	int i;
	for(i=0; i< (n & (-4)); i+=4) {
        double2 a2_v1 = *(double2*)(&a[i]);
        double2 b2_v1 = *(double2*)(&b[i]);
        double2 a2_v2 = *(double2*)(&a[i+2]);
        double2 b2_v2 = *(double2*)(&b[i+2]);
	    sum2_v1 += a2_v1*b2_v1;
        sum2_v2 += a2_v2*b2_v2;
	}
...
double dot_double4(int n, double *a, double *b ) {
	...
	int i;
	for(i=0; i< (n & (-4)); i+=4) {
        double4 a4 = *(double4*)(&a[i]);
        double4 b4 = *(double4*)(&b[i]);
	    sum4 += a4*b4;
	}
