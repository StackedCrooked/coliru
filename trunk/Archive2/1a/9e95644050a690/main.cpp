...
int main () 
{
  ...
  { // Inner block's scope begins here.
    ...
    int x{50}; // Here within this inner block's scope, another new (and totally
               //  different) 'x' variable initialization is OK, since we're
               //  protected here inside main()'s inner block from
               //  having an 'x' name clash occur in our program.
  ...
  } // Inner block's scope ends here, so does the lifetime of the inner 'x'.
