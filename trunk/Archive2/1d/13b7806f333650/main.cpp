...
inline double mag(double *li, int n) {
	double s = 0;
	for (int i = 0; i < n; i++) {
		s += li[i]*li[i];
	}
...
double mag_SSE(double *li, int n) {
	__m128d s2;
	int i;
	double s;
	...
	for (i = 0; i < (n & (-2)); i+=2) {
		__m128d li2;
		li2 = _mm_loadu_pd(&li[i]);
		s2 = _mm_add_pd(_mm_mul_pd(li2, li2), s2);
	}
	double out[2];
	_mm_storeu_pd(out, s2);
	s = out[0] + out[1];
	for(;i<n; i++) {
		s += li[i]*li[i];
	}
...
double inner_sum_SSE(double *li, double *lj, int n) {
	__m128d s2;
	int i;
	double s;
	...
	for (i = 0; i < (n & (-2)); i+=2) {
		__m128d li2, lj2;
		li2 = _mm_loadu_pd(&li[i]);
		lj2 = _mm_loadu_pd(&lj[i]);
		s2 = _mm_add_pd(_mm_mul_pd(li2, lj2), s2);
	}
	double out[2];
	_mm_storeu_pd(out, s2);
	s = out[0] + out[1];
...
double *cholesky6(double *A, int n) {
    double *L = (double*)calloc(n * n, sizeof(double));
    if (L == NULL)
        exit(EXIT_FAILURE);
 	
	for (int j = 0; j <n; j++) {
		double s = mag_SSE(&L[j * n], j);
		L[j * n + j] = sqrt(A[j * n + j] - s);
		#pragma omp parallel for schedule(static)
		for (int i = j+1; i <n; i++) {
            double s = inner_sum_SSE(&L[j * n], &L[i * n], j);
            L[i * n + j] = (1.0 / L[j * n + j] * (A[i * n + j] - s));
        }
	}
    return L;
...
double *cholesky7(double *A, int n) {
    double *L = (double*)calloc(n * n, sizeof(double));
    if (L == NULL)
        exit(EXIT_FAILURE);
...
double *cholesky8(double *A, int n) {
    double *L = (double*)calloc(n * n, sizeof(double));
    if (L == NULL)
        exit(EXIT_FAILURE);
