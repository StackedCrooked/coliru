...
int main()
{
	...
	Gizmo <unsigned> g (42);
	g.DoIt (l.begin(), l.end()); // list<string> works
    
    const char* const char_pointers[] = {"foo", "bar", "baz"};
    g.DoIt (begin(char_pointers), end(char_pointers)); // const char*[] works
    
    typedef int battelship;
    vector<battleship> armada{1,2,3,4};
    g.DoIt (begin(armada), end(armada)); // vector<battleship> is rejected.
}
