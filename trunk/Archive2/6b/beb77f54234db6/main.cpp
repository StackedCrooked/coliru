...
struct Stack
{
    ...
    // (1) Internal: only accessible from the scheduler thread
    ...
    // (2) std::future: schedules "addProtocol" as a task and returns
    // a future result
    std::future<Protocol&> addProtocol() {
        TRACE
        return schedule([=](Internal internal) -> Protocol&{
            return addProtocol(internal);
        });
    }

    // (3) callback: schedules a callback to be called on scheduler thread
    // inside the callback other internal methods may be called
    // returns the return type of the callback as a std future
    template<typename F>
...
{
    TRACE
    return stack_->schedule([=](Internal internal) -> Session&{
