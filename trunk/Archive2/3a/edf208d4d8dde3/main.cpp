...
int GetMaxAge(std::vector<Person> &guys){
/*
      // Scala version
...
*/
    ...
    {
        const int max_age = *boost::max_element(guys | boost::adaptors::transformed(
            std::function<int(Person)>([](Person const &p){return p.age;})));
    }
    {
        const int max_age = boost::accumulate(guys, -1, 
...
int main()
{
    ...
    std::vector<std::string> words = {
        "Max age", boost::lexical_cast<std::string>(GetMaxAge(guys))
    };
