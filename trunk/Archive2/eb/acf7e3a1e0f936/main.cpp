...
using member_field_type = 
      tuple_element<i, decltype(make_tuple( typedef<T requires is_member_object_pointer>... ))>;
*/
template<typename T, int i> using member_field_type = 
      tuple_element<i, decltype(make_tuple( TYPEDEF_IS_MEMBER_FIELD(T) ))>;

/*
template<typename T, int i> 
typename member_field_type<T,i>::type member_field = 
         std::get<i>(std::make_tuple( typedef<T requires is_member_object_pointer>... ) ); 
*/
template<typename T, int i> typename member_field_type<T,i>::type member_field = 
...
template<typename T, int i> 
using member_field_type = 
      tuple_element<i, decltype(make_tuple( typename<T requires is_member_object_pointer>... ))>;
*/
template<typename T, int i> const char* member_field_nm
...
int main() {
    ...
    //for(auto &foos : {typedef<T requires is_same<double(T*::)()>>...}) {
