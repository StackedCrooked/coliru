...
namespace date_time {
  ...
  struct time_formats {
    public:
      ...
      static const char_type fractional_seconds_4digits[3];              // 3
  ...
  time_formats<CharT>::fractional_seconds_4digits[3] = {'%','4'};
  ...
  class time_facet :
   ...
   public:
    ...
    static const char_type* fractional_seconds_4digits;               // %4
                ...
                const time_type& time_arg) const
    {
      ...
      if (local_format.find(fractional_seconds_4digits) != string_type::npos) {
        // replace %4 with nnn
        if (frac_str.empty()) {
          frac_str = fractional_seconds_as_3digit_string(time_arg.time_of_day(), false);
        }
        boost::algorithm::replace_all(local_format,
                                      fractional_seconds_4digits,
                                      frac_str);
      }
                ...
                const time_duration_type& time_dur_arg) const
    {
      ...
      if (format.find(fractional_seconds_4digits) != string_type::npos) {
        // replace %4 with nnn
        if (frac_str.empty()) {
  ...
  protected:
                                 ...
                                 bool null_when_zero)
    {
      ...
      for (auto n = time_arg.num_fractional_digits(); n>4; --n)
          frac_sec /= 10;
  ...
  time_facet<time_type, CharT, OutItrT>::fractional_seconds_4digits = time_formats<CharT>::fractional_seconds_4digits;
