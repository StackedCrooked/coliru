...
struct Map
{
    ...
    U& operator[](UniqueIndex<T>& t)
    {
        auto required_size = t.getIndex() + 1u;
        if (mData.size() < required_size)
        {
            mData.resize(required_size);
        }
        return mData[t.getIndex()];
    }
    
    void erase(UniqueIndex<T>& t)
    {
        mData[t.getIndex()].clear();
    }
