...
struct Processor
{
    ...
    Processor(ProcessorType processor) : mImpl(std::make_shared<Impl<ProcessorType>>(std::move(processor)))
    {
    }
    
    // Provides allocator argument.
    // This enables you to use a memory pool that keeps the processors close together in memory.
    template<typename Alloc, typename ProcessorType>
    Processor(std::allocator_arg_t, Alloc alloc, const ProcessorType& processor) :
        mImpl(std::allocate_shared<Impl<ProcessorType>>(alloc, processor))
    {
    }
...
private:
    ...
    {
        template<typename Snapshot>
        Wrapper(Snapshot& inSnapshot) :
            mTypeIndex(typeid(Snapshot)),
            mPtr(&inSnapshot)
        {
        }
    ...
    template<typename T>
    struct Impl final : AbstractImpl
    {       
        Impl(T&& t) :
            t(std::move(t))
        {
        ...
        void get_snapshot(Wrapper wrapper)
        {
    ...
    std::shared_ptr<AbstractImpl> mImpl;
};
