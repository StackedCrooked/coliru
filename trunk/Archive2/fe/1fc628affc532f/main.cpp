...
template<class... Ts, class F, size_t... Is>
void unpack(F&& f, uint8_t *dataOffset, std::index_sequence<Is...>){
    f(unpack_arg<Is, Ts...>(dataOffset)...);
}

template<class... Ts>
void unpack(std::function<void(Ts...)> f, uint8_t *dataOffset){
    return unpack<Ts...>(f, dataOffset, std::index_sequence_for<Ts...>());
}

template<class R, class... Ts>
void unpack(R (*f)(Ts...), uint8_t *dataOffset){
    return unpack<Ts...>(f, dataOffset, std::index_sequence_for<Ts...>());
}
