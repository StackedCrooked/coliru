...
private:
    void destroyChild(AbstractWidget* child)
    {
        mChildren.erase(std::remove_if(mChildren.begin(), mChildren.end(), [child](AbstractWidget& element) { return &element == child; }), mChildren.end());
    }

    virtual void do_finalize() {}

    AbstractWidget* mParent;
    boost::ptr_vector<AbstractWidget> mChildren;
};
