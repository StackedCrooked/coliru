...
template<typename T>    std::string  mapperC2CXX(       char* t ) { return std::string(t); }
template<typename T>    std::string  mapperC2CXX( const char* t ) { return std::string(t); }
template<typename T>    Object       mapperC2CXX( PyObject*   p ) { return Object{p}; }
...
//#define BIND(c_slot, cxx_target) c_slot = & Generate< decltype(c_slot), &c_slot,   decltype(cxx_target), &cxx_target >::call;

int main()
{
//    BIND( table.tp_getattr, Base::getattr );
        
    table.tp_getattr = & Generate< getattrfunc, decltype(&Base::getattr), &Base::getattr >::call;
