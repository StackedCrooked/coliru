...
enum class NetworkListener { NLState_Unspecified, NLState_Running };

struct TCPNetworkListener {

    boost::asio::io_service m_IOService;
    tcp::endpoint m_TCPEndpoint = *tcp::resolver(m_IOService).resolve({ "localhost", "6767" });
    tcp::acceptor m_SocketAcceptor { m_IOService };
    long m_lastConnectedClientId = -1;
    NetworkListener m_listenerState = NetworkListener::NLState_Unspecified;
...
void TCPNetworkListener::ClientConnected(SharedPtr const& session) const {
    std::cout << __PRETTY_FUNCTION__ << " id " << session->m_clientId << "\n";
}
...
void TCPNetworkListener::ClientDataReceived(SharedPtr const& session, std::string data) const {
    std::cout << __PRETTY_FUNCTION__ << " id " << session->m_clientId << ": '" << data << "'\n";
}
