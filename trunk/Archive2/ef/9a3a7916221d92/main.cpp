...
std::unique_ptr<Base> CreateStuff()
{
    return std::make_unique<Child>();
}

std::vector<std::unique_ptr<Base>> CreateListOfStuff()
{
    std::vector<std::unique_ptr<Base>> result;
    result.reserve(3);
    for(int i = 0; i < 3; i++)
    {
        result.emplace_back(CreateStuff());
    }
    return result;
}
...
int main()
{
  const std::vector<std::unique_ptr<Base>> the_list = CreateListOfStuff();
