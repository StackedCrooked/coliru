...
int main()
{
    char str [31] = "0123dF020 aaaaaaaaaaa bbbbbbbb";
...
void PrintWordsSorted(const char * str)
{
    ...
    for ( i = 0 ; i < l + 1 ; i++)
    {   if ( *(str+i) == ' ' || *(str+i) == '\0')
        {   *(substr+actualWord) = (char*) malloc ( i - startingWord + 1 );
            for ( j = 0 ; j < actualWordLength ; j++ ) // Recopy
            {   *(*(substr+actualWord)+j) = *(str+startingWord+j);
            }
            *(*(substr+actualWord)+actualWordLength) = '\0';
            *(scores+actualWord) = SumCharsAndDigits2(*(substr+actualWord)); // I used a trick here :D
            actualWord++;
            actualWordLength = 0;
            startingWord = i+1;
            continue;
    ...
    // Sorry time was running short so i improvised an ugly way of dealing with it
    for ( i = 0 ; i < nWord ; i++)
