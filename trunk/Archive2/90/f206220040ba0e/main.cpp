...
template<class F>
void set_deduced(std::true_type, F&& f)
{
    set_i( types<function_signature_t<decltype(&unqualified_t<F>::operator())>>( ), std::forward<F>(f) );
}

template<class F>
void set_deduced(std::false_type, F&&){}

template<class F>
void set_i( types<>, F&& f ) {
    std::cout << "traited ";
    using Fty = unqualified_t<F>;
    static constexpr auto deducible = has_deducible_signature<Fty>();
    static_assert(deducible, "some useful error message");
    set_deduced(deducible, std::forward<F>(f));
}
