...
public:
    ...
    bi_vec(size_type n, reference r, allocator_type a) :allocator_type(std::move(a)), buffer(nullptr), first(nullptr), last(nullptr), endbuf(nullptr) {_insert(first, make_ref_iter(r,0), make_ref_iter(r, n));}
    template<class InIter> bi_vec(InIter f, InIter l) :buffer(nullptr), first(nullptr), last(nullptr), endbuf(nullptr) {_insert(first, f, l);}
    ...
    template<class iter> iterator insert(const_iterator loc, iter f, iter l) {assert(_valid(loc)); return _insert(loc, f, l);}
    ...
    template<class iter> void assign(iter f, iter l) {_erase(first, last); _insert(buffer, f, l);}
