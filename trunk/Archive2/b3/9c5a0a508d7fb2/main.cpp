...
void test_phrase_parser_attr(char const *input, P const &p, T &attr, bool full_match = true) {
...
BOOST_FUSION_ADAPT_STRUCT(person,
                          (std::string, name)(int, age)(double, size)(std::vector<std::string>, favorite_colors))
...
int main() {
    ...
    // keyword_list
    {
        ...
        // Let's declare a small list of people for which we want to collect
        // information.
        person John, Mary, Mike, Hellen, Johny;
        test_phrase_parser_attr("name = \"John\" \n age = 10 \n size = 1.69m ", no_constraint_person_rule, John); // full in orginal order
        std::cout << John;

        test_phrase_parser_attr("age = 10 \n size = 1.69m \n name = \"Mary\"", no_constraint_person_rule, Mary); // keyword oder doesn't matter
        std::cout << Mary;

        test_phrase_parser_attr("size = 1.69m \n name = \"Mike\" \n age = 10 ", no_constraint_person_rule, Mike); // still the same result
        ...
        */
        ...
        /*The parser definition below uses the kwd directive occurence
         * constraint variants to make sure that the name and age keyword occur
         * only once and allows the favorite color entry to appear 0 or more
         * times. */
        ...
        test_phrase_parser_attr(
            ...
            constraint_person_rule, Hellen);
        std::cout << Hellen;

        // Parsing this string will fail because the age and size minimum occurence requirements aren't met.
        test_phrase_parser_attr("name = \"Johny\"  \n favorite color = \"blue\" \n favorite color = \"green\" ",
                                constraint_person_rule, Johny);

        /* Parsing the first string will succeed but fail for the second string
         * as the occurence constraints aren't met. This code should
         * print:[teletype]
