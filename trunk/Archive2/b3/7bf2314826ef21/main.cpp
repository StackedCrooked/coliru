...
Board LoopMoves(Board& b) {
    for (int x = 0; x<b.move; x++) {
        b.push_back(Move(b,x));
        }
    b.move=0;    
//std::for_each(b0.begin(), b0.end(), LoopMoves); //only adds moves if not listed already 
...
std::for_each(b0.begin(), b0.end(), PrintIt); std::cout<<"********\n";
    ...
    std::for_each(b0.begin(), b0.end(), LoopMoves); //b.move get set to 0 on exit so no moves after 1st
