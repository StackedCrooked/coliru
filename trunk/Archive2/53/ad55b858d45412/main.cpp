...
{
    // end iterator
    SkipIterator(It it) : cur_it_(it), cur_idx_(0), skip_list_()
    {    
    }
    
    // iterator + array 
    SkipIterator(It it, int (&indices)[N]) : cur_it_(it), cur_idx_(0), skip_list_()
    {    
        auto b = indices;
        auto e = b + N;
...
auto operator|(Range&& range, skip_t<Count> s)
{
    ...
    return make_range(skipper(begin(range), s.n_), skipper(end(range)));
}
...
int main()
{    
    std::set<int> numbers = { 0, 1, 2, 3, 4 };
    
    for (auto n : make_range(numbers) | skip(0, 2, 4))
    {
        std::cout << n << ' ';
    }
}
