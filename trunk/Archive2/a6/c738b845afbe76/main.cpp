...
struct decorate_rec
{
	template <typename A, typename N = result<decorate_rec<D...>(A)>>
	E<N> operator()(A&& a) const
	{
		return E<N>(decorate_rec<D...>()(std::forward<A>(a)));
	}
};

template <template <typename> class E>
struct decorate_rec <E>
{
...
{
	template <typename A, typename N = result<decorate_rec<D...>(A)>>
	std::shared_ptr<E<N>> operator()(A&& a) const
	{
		return std::shared_ptr<E<N>>(new E<N>(decorate_rec<D...>()(std::forward<A>(a))));
	}
};

template <template <typename> class E, template <typename> class... D, typename A>
result <decorate_impl<E, D...>(A)> decorate(A&& a)
{
	return decorate_impl<E, D...>()(std::forward<A>(a));
}
