...
public:
    ...
    //! in one central place. This gives you the chance to rethrow them as a type
    //! appropriate for the user domain.
    ...
    R try_deserialize_result(const std::string & maybe)
    {
        ...
        switch (result_code)
        {
            ...
            case ResultCode::Failure:
            {
                ...
                {
                    handle_error(exc); // should rethrow a new exception!
                    throw exc; // rethrow original if handler doesn't throw anything
                }
            }
            default:
            {
