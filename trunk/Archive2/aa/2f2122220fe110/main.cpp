...
int main()
{
  const size_t amount_of_random_numbers = 1024*1024*32;
  // Engines
  const size_t Nrng = 16;
  ...
  vector<vector<double>::iterator> begins(Nrng);
  for(size_t i=0; i<Nrng; ++i)
    begins[i] = rngs.begin() + i*amount_of_random_numbers/Nrng;
  
  vector<vector<double>::iterator> ends(Nrng);
  for(size_t i=0; i<Nrng; ++i)
    ends[i] = rngs.end() - (Nrng-i)*amount_of_random_numbers/Nrng;
