...
struct kanmusu;
string simulate(const vector<kanmusu>&, const vector<kanmusu>&);	//シミュレート
struct condition;
void AttackAction(const int Target, const int BaseAttack, const vector<kanmusu> &Yours, vector<int> &YoursAlived,
...
struct kanmusu{	// メンバ変数
	...
	//コンストラクタ
	kanmusu(const string, const SC, const int, const int, const int, 
...
// コンストラクタ
kanmusu::kanmusu(const string name, const SC kind, const int slots,
	...
	while(1){
				...
				if(MineCond[i].NowAirs[j] != 0){
					if(YoursAlived.size() != 0){
						if(Mine[i].Weapons[j].Kind == "艦攻"){
							...
							AttackAction(Target, BaseAttack, Yours, YoursAlived, YoursCond, MineForm, BattlePosition,
				...
				if(YoursCond[i].NowAirs[j] != 0){
					if(MineAlived.size() != 0){
						if(Yours[i].Weapons[j].Kind == "艦攻"){
							...
							AttackAction(Target, BaseAttack, Mine, MineAlived, MineCond, YoursForm, BattlePosition,
		...
		for(int i = 0; i < MineShips; ++i){
			...
			if(Mine[i].Kind & isTorpedo){
				...
				for(int i = 0; i < YoursAlived.size(); ++i){
					if((Yours[YoursAlived[i]].Kind != SC_SS) && (Yours[YoursAlived[i]].Kind != SC_SSV)){
						YoursAlivedWithoutSS.push_back(YoursAlived[i]);
					}
				}
		...
		for(int i = 0; i < YoursShips; ++i){
			...
			if(Yours[i].Kind & isTorpedo){
				...
				for(int i = 0; i < MineAlived.size(); ++i){
					if((Mine[MineAlived[i]].Kind != SC_SS) && (Mine[MineAlived[i]].Kind != SC_SSV)){
						MineAlivedWithoutSS.push_back(MineAlived[i]);
					}
				}
...
void AttackAction(const int Target, const int BaseAttack, const vector<kanmusu> &Yours, vector<int> &YoursAlived,
	...
	if(Damage >= 1){
		if(Rand(mt) * 100 >= EvadeSum){	//唐突な「回避」要素
...
int main(){
	...
	//艦娘
	kanmusu Fubuki_Fubuki("吹雪 Lv1", SC_DD, 2, 15, 10, 27, 10, 5, 40, 20, 5, REACH_SHORT, 17, SPEED_HIGH);
		Fubuki_Fubuki.Weapons[0] = Gun_127_Multi;
		Fubuki_Fubuki.Weapons[1] = Torpedo_61_3;
		Fubuki_Fubuki.Airs[0] = 0;
		Fubuki_Fubuki.Airs[1] = 0;
	kanmusu Kuma_Oi("大井改 Lv10", SC_CLT, 2, 32, 8, 80, 13, 14, 37, 28, 11, REACH_MEDIUM, 10, SPEED_HIGH);
		Kuma_Oi.Weapons[0] = Torpedo_61_4_Oxy;
		Kuma_Oi.Weapons[1] = Torpedo_61_4_Oxy;
		Kuma_Oi.Airs[0] = 0;
		Kuma_Oi.Airs[1] = 0;
	kanmusu Mogami_Mogami("最上改 Lv10", SC_CAV, 4, 50, 28, 23, 24, 39, 38, 0, 25, REACH_MEDIUM, 10, SPEED_HIGH);
	...
	kanmusu Akagi_Akagi("赤城 Lv1", SC_CV, 4, 69, 0, 0, 32, 28, 28, 0, 44, REACH_SHORT, 12, SPEED_HIGH);
	...
	kanmusu Ise_Ise("伊勢改 Lv10", SC_BBV, 4, 77, 63, 0, 45, 75, 36, 0, 22, REACH_LONG, 30, SPEED_LOW);
		...
		Ise_Ise.Airs[2] = 11;
		Ise_Ise.Airs[3] = 14;
	kanmusu E_E401("伊401(い401) Lv1", SC_SSV, 1, 20, 2, 36, 0, 5, 13, 0, 15, REACH_SHORT, 20, SPEED_LOW);
		E_E401.Weapons[0] = None;
		E_E401.Airs[0] = 3;

	// 情報表示
	Fubuki_Fubuki.putInfo();
	Kuma_Oi.putInfo();
	Mogami_Mogami.putInfo();
	Akagi_Akagi.putInfo();
	Ise_Ise.putInfo();
	E_E401.putInfo();
	Gun_127_Multi.putInfo();
	Gun_203_Multi.putInfo();
	Gun_356_Multi.putInfo();
	Torpedo_61_3.putInfo();
	Torpedo_61_4_Oxy.putInfo();
	WB_Zuiun.putInfo();
	WS_Zeroshiki.putInfo();
	PF_Zero_21.putInfo();
	PB_99.putInfo();
	PA_97.putInfo();
	None.putInfo();
	...
	//vector<kanmusu> First_Kantai{Fubuki_Fubuki, Fubuki_Murakumo, Mogami_Mogami};
	//vector<kanmusu> Second_Kantai{Fubuki_Murakumo};

	//対戦
	cout << simulate(First_Kantai, Second_Kantai, FOR_TRAIL, FOR_TRAIL) << "\n";
	cout << simulate(Second_Kantai, First_Kantai, FOR_TRAIL, FOR_TRAIL) << "\n";
}
...
○対空装備による迎撃結果(未実装)
○攻撃結果
　(自軍)最上改 Lv10の瑞雲が伊勢改 Lv10に攻撃！
　　残り耐久：66(健在)
　(自軍)赤城 Lv1の九九式艦爆が赤城 Lv1に攻撃！
　　残り耐久：62(健在)
　(自軍)赤城 Lv1の九七式艦攻が伊勢改 Lv10に攻撃！
　　残り耐久：0(撃沈)
　(自軍)伊勢改 Lv10の瑞雲が赤城 Lv1に攻撃！
　　残り耐久：54(健在)
　(敵軍)赤城 Lv1の九九式艦爆が赤城 Lv1に攻撃！
　　残り耐久：61(健在)
　(敵軍)赤城 Lv1の九七式艦攻が最上改 Lv10に攻撃！
　　残り耐久：50(健在)
　(敵軍)最上改 Lv10の瑞雲が赤城 Lv1に攻撃！
　　残り耐久：55(健在)
【開幕雷撃フェイズ】
　(自軍)伊401(い401) Lv1が赤城 Lv1に攻撃！
　　残り耐久：54(健在)
　(敵軍)伊401(い401) Lv1が大井改 Lv10に攻撃！
　　残り耐久：29(健在)
【交戦形態フェイズ】
結果：反航戦
【砲撃戦フェイズ】
　(自軍)伊勢改 Lv10が吹雪 Lv1に攻撃！
　　残り耐久：13(健在)
　(敵軍)最上改 Lv10が伊401(い401) Lv1に攻撃！
　　残り耐久：18(健在)
　(自軍)最上改 Lv10が最上改 Lv10に攻撃！
　　残り耐久：46(健在)
　(敵軍)吹雪 Lv1が伊401(い401) Lv1に攻撃！
　　残り耐久：15(小破)
　(自軍)大井改 Lv10が最上改 Lv10に攻撃！
　　残り耐久：46(健在)
　(敵軍)吹雪 Lv1が伊401(い401) Lv1に攻撃！
　　残り耐久：12(小破)
　(自軍)吹雪 Lv1が吹雪 Lv1に攻撃！
...
　(敵軍)吹雪 Lv1が伊401(い401) Lv1に攻撃！
　　残り耐久：9(中破)
　(自軍)伊401(い401) Lv1が吹雪 Lv1に攻撃！
　　残り耐久：12(健在)
　(自軍)吹雪 Lv1が伊401(い401) Lv1に攻撃！
　　残り耐久：17(健在)
　(敵軍)伊401(い401) Lv1が伊401(い401) Lv1に攻撃！
　　残り耐久：7(中破)
　(自軍)大井改 Lv10が伊401(い401) Lv1に攻撃！
　　残り耐久：15(小破)
　(敵軍)赤城 Lv1が大井改 Lv10に攻撃！
　　残り耐久：25(健在)
　(自軍)最上改 Lv10が伊401(い401) Lv1に攻撃！
　　残り耐久：13(小破)
　(敵軍)最上改 Lv10が伊401(い401) Lv1に攻撃！
　　残り耐久：6(中破)
　(自軍)赤城 Lv1が赤城 Lv1に攻撃！
　　残り耐久：54(健在)
　(敵軍)大井改 Lv10が伊401(い401) Lv1に攻撃！
　　残り耐久：5(大破)
　(自軍)伊勢改 Lv10が伊401(い401) Lv1に攻撃！
　　残り耐久：12(小破)
　(敵軍)吹雪 Lv1が伊401(い401) Lv1に攻撃！
　　残り耐久：4(大破)
　(自軍)伊401(い401) Lv1が最上改 Lv10に攻撃！
　　残り耐久：42(健在)
【雷撃戦フェイズ】
　(自軍)吹雪 Lv1が大井改 Lv10に攻撃！
　　残り耐久：32(健在)
　(自軍)大井改 Lv10が最上改 Lv10に攻撃！
　　残り耐久：42(健在)
　(自軍)最上改 Lv10が吹雪 Lv1に攻撃！
　　残り耐久：12(健在)
　(敵軍)伊401(い401) Lv1が大井改 Lv10に攻撃！
　　残り耐久：22(小破)
　(敵軍)最上改 Lv10が赤城 Lv1に攻撃！
　　残り耐久：55(健在)
　(敵軍)大井改 Lv10が吹雪 Lv1に攻撃！
　　残り耐久：15(健在)
　(敵軍)吹雪 Lv1が赤城 Lv1に攻撃！
　　残り耐久：48(小破)
【夜戦フェイズ】
　(自軍)吹雪 Lv1が伊401(い401) Lv1に攻撃！
　　残り耐久：10(中破)
　(敵軍)伊401(い401) Lv1が赤城 Lv1に攻撃！
　　残り耐久：42(小破)
　(自軍)大井改 Lv10が伊401(い401) Lv1に攻撃！
　　残り耐久：9(中破)
　(自軍)最上改 Lv10が最上改 Lv10に攻撃！
　　残り耐久：42(健在)
　(敵軍)最上改 Lv10が伊勢改 Lv10に攻撃！
　　残り耐久：77(健在)
　(自軍)伊勢改 Lv10が最上改 Lv10に攻撃！
　　残り耐久：36(小破)
　(敵軍)吹雪 Lv1が伊401(い401) Lv1に攻撃！
　　残り耐久：3(大破)
【結果表示】
...
　　赤城 Lv1 42/69(小破)
　　伊勢改 Lv10 77/77(健在)
　　伊401(い401) Lv1 3/20(大破)

　敵軍：
　　伊401(い401) Lv1 9/20(中破)
　　伊勢改 Lv10 0/77(撃沈)
　　赤城 Lv1 54/69(健在)
　　最上改 Lv10 36/50(小破)
　　大井改 Lv10 32/32(健在)
...
○対空装備による迎撃結果(未実装)
○攻撃結果
　(自軍)伊勢改 Lv10の瑞雲が伊401(い401) Lv1に攻撃！
　　残り耐久：20(健在)
　(自軍)赤城 Lv1の九九式艦爆が伊401(い401) Lv1に攻撃！
　　残り耐久：20(健在)
　(自軍)赤城 Lv1の九七式艦攻が吹雪 Lv1に攻撃！
　　残り耐久：15(健在)
　(自軍)最上改 Lv10の瑞雲が大井改 Lv10に攻撃！
　　残り耐久：28(健在)
　(敵軍)最上改 Lv10の瑞雲が伊401(い401) Lv1に攻撃！
　　残り耐久：20(健在)
　(敵軍)赤城 Lv1の九九式艦爆が伊401(い401) Lv1に攻撃！
　　残り耐久：20(健在)
　(敵軍)赤城 Lv1の九七式艦攻が伊勢改 Lv10に攻撃！
　　残り耐久：12(大破)
　(敵軍)伊勢改 Lv10の瑞雲が大井改 Lv10に攻撃！
　　残り耐久：32(健在)
【開幕雷撃フェイズ】
　(自軍)伊401(い401) Lv1が最上改 Lv10に攻撃！
　　残り耐久：50(健在)
　(敵軍)伊401(い401) Lv1が吹雪 Lv1に攻撃！
　　残り耐久：12(健在)
【交戦形態フェイズ】
結果：同航戦
【砲撃戦フェイズ】
...
　　残り耐久：8(中破)
【雷撃戦フェイズ】
　(自軍)最上改 Lv10が伊勢改 Lv10に攻撃！
　　残り耐久：77(健在)
　(自軍)大井改 Lv10が最上改 Lv10に攻撃！
　　残り耐久：35(小破)
　(自軍)吹雪 Lv1が伊勢改 Lv10に攻撃！
　　残り耐久：67(健在)
　(敵軍)吹雪 Lv1が赤城 Lv1に攻撃！
　　残り耐久：52(健在)
　(敵軍)大井改 Lv10が吹雪 Lv1に攻撃！
　　残り耐久：10(小破)
　(敵軍)最上改 Lv10が最上改 Lv10に攻撃！
　　残り耐久：44(健在)
【夜戦フェイズ】
　(自軍)伊401(い401) Lv1が伊勢改 Lv10に攻撃！
　　残り耐久：62(健在)
　(敵軍)吹雪 Lv1が伊401(い401) Lv1に攻撃！
　　残り耐久：6(中破)
　(自軍)最上改 Lv10が吹雪 Lv1に攻撃！
