...
struct Future
{
    ...
    {
        return std::async(std::launch::deferred, [this](F f) {
            return f(mFuture.get());
        }, std::forward<F>(f));
    }

    template<typename F>
    auto finally(F&& f) -> std::future<void>
    {
        // async launch policy => blocking destructor
        return std::async(std::launch::async, [this](F f){
            f(mFuture.get());
        }, std::forward<F>(f));
    }
