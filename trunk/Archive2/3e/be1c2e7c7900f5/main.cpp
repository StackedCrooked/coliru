...
R unpack(std::function<R(Args...)> f, const std::uint8_t* from) {
  return unpack<Args...>(std::move(f), from);
}

// Deduce argument types from function pointer
template <typename R, typename...Args>
R unpack(R (*f)(Args...), const std::uint8_t* from) {
  return unpack<Args...>(f, from);
}
...
int main()
{
  ...
  std::function<void (uint8_t, int8_t, uint16_t, int16_t, uint32_t, int32_t, uint64_t, int64_t, float, double, std::vector<double> const&)> ffoo1 = foo1;
  std::function<void (uint8_t, int8_t, uint16_t, int16_t, uint32_t, int32_t, int64_t, uint64_t, float, double, std::vector<double> const&)> ffoo2 = foo2;
