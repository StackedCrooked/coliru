...
namespace camper
{
	...
	{
		static constexpr std::common_type_t<decltype(Pairs::first)...> pairs[][2] = { { Pairs::first, Pairs::second }... };
		for ( int i = 0; i != sizeof...(Pairs); ++i )
			Arcoth::compare_swap( begin[pairs[i][0]], begin[pairs[i][1]] );
	}
...
int main()
{
	auto constexpr size = 64, inner = 2048, outer = 5000;
