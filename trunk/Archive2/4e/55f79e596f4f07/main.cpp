...
namespace TupleVisit{
        ...
        namespace details{
            ...
            template<>
            struct visitIndices< meta::list<> >{
                template<typename V, typename T>
                static void apply(V && v, T & t){} 
            }
        
            template<typename I, typename... O>
            struct visitIndices< meta::list<I,O...>>{
                template<typename V, typename T>
                static void apply(V && v, T & t){
                    v( std::get<I::value>(t) );
                    visitIndices< meta::list<O...> >(v,t);    
                }
            }
            
            template<typename... Types, typename Visitor, typename... T>
            static void visitSpecific(Visitor && v, std::tuple<T...> & t){
                
                // make index list of the elements in List and the tuple 
                using types = meta::list<Types...>;
                using tupleTypes = meta::list<T...>;
                
                using indices = remove< meta::npos,  
                                    meta::transform< types, 
                                                     meta::bind_back<meta::quote<meta::find_index>,tupleTypes>
                                    >
                                >;
                
                // Run Visitor for each index
                visitIndices<indices>::apply(v,t);
            }
        ...
        template<typename... Types, typename Visitor, typename... T>
        void visitSpecific(Visitor && v, std::tuple<T...> & t){
            details::visitSpecific<Types...>(v,t);
        }
...
int main()
{
    ...
    TupleVisit::visitSpecific(vis{},a);
