...
struct Base2 { virtual void foo() = 0; }; // polymorphic
struct Derived2 : public Base2 { void foo() {} };
...
int main()
{
    ...
    Derived d1;
    Base& b1 = d1;
    std::cout << "-reference to d1: " << typeid(d1).name() << '\n';
    std::cout << "reference to non-polymorphic base: " << typeid(b1).name() << '\n';
 
    Derived2 d2;
    Base2& b2 = d2;
    std::cout << "-reference to d2: " << typeid(d2).name() << '\n';
