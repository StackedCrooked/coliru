...
struct graph_builder
{
    ...
    void operator()(VertexDescriptor src, VertexDescriptor tgt, float capacity)
    {
        bool inserted;
        EdgeDescriptor edge;
        boost::tie(edge,inserted) = add_edge(src,tgt,index++,g);
        if(inserted) // if the edge was inserted set capacity, add the reverse edge with capacity 0, and fill the reverse_vec
        {
            ...
            capacity_vec.push_back(0.f);
            reverse_vec.push_back(edge);
            
        }
        else //if the edge wasn't inserted you added an edge that is the reverse of one that already exists, simply update the capacity
        {
            capacity_vec[get(boost::edge_index, g)[edge]]=capacity;
        }
    }
...
int main()
{
  ...
  {
      builder(i,sourceId,2.f);

      builder(i,sinkId,2.f);
  }
