...
struct Vector {
	...
	template <typename... Tn, typename = type<std::enable_if<(sizeof...( Tn ) > 0) && (sizeof...( Tn ) <= n), int>>>
	Vector( Tn&&... argn ) {
		std::size_t idx = 0;
		using swallow = int[];
		(void)swallow{ 0, ( void(
	...
	template <typename... Tn, typename = type<std::enable_if<sizeof...( Tn ) == (n + 1)>>>
	Vector( Tn&&... argn ) {
		std::size_t idx = 0;
		auto args = std::make_tuple( std::forward<Tn>( argn )... );
		set( args, make_indices<sizeof...( Tn )-1>( ) );
		// Now divide my last elem...
		for ( auto& r : res ) {
			r /= std::get<sizeof...(Tn)-1>( args );
		}
	}
	...
	void set( std::false_type, std::size_t& idx, Tr&& r ) {
		...
		for ( const auto& elem : r ) {
			set( std::true_type( ), idx, static_cast<T>( elem ) );
		}
	}

	template <typename Tu, std::size_t... I>
	void set( Tu&& tuple, indices<I...> ) {
		std::size_t idx = 0;
		using swallow = int[];
		(void)swallow{ 0, ( void(
			set( std::true_type( ), idx, std::get<I>( tuple ) )
			), 0 )... };
	}
