...
struct value
{
    ...
    static constexpr decltype(auto) apply(A1&& a1) { return std::forward<A1>(a1); }
...
struct wrap
{
    ...
    static constexpr auto func(Ts&&... ts)
    {
        return EXPR::apply(std::forward<Ts>(ts)...);
    }
};
...
int main()
{
    ...
    // (4 + 5) + (2 * 6) = 9 + 12 = 21
    std::cout << wrap<plus<plus<value, value>, multiply<value, constant<int, 6>>>>::func(4, 5, 2) << std::endl;
        
    // (((1 * 2) * 3) * 4) = 24
    std::cout << wrap<multiply<multiply<multiply<value, value>, value>, value>>::func(1, 2, 3, 4) << std::endl;
}
