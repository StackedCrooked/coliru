...
#define LOG(level) if (level < gLogLevel) ; else Log(level) << __FILE__ << ":" << __LINE__ << ": "
...
struct Adaptor
{
    ...
    struct Allocator
    {
        ...
        void construct(const U * const p, Args&& ...args) const
        {
            assert(p);
        ...
        {
            Log(High) << "Allocation size=" << (n * sizeof(T)) << " bytes. Type=" << Demangle(typeid(T).name());
            assert(n);
            assert(mStorage);
...
struct Mutex
{
    ...
    Mutex& operator=(const Mutex&) noexcept { return *this; }
...
struct FixedFreeListPool
{
    ...
    FixedFreeListPool(FixedFreeListPool&&) noexcept = default;
    FixedFreeListPool& operator=(FixedFreeListPool&&) noexcept = default;
...
template<typename T>
using Allocator = std::scoped_allocator_adaptor<Adaptor<FlexiblePool>::Allocator<T>>;
...
int main()
{
    std::thread([]{ std::this_thread::sleep_for(std::chrono::milliseconds(1000));  LOG(Low) <<  "FORCE QUIT"; std::abort(); }).detach();
    TaskQueue<int> task_queue(1);
    LOG(Low) << __LINE__;
