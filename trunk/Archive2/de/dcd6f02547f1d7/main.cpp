...
constexpr int sum_multiples_of_n_lt_m( int m, int n ) { return sum_first_m_multiples_of_n( n, (m-1)/n ) ; }
constexpr int sum_multiples_of_a_or_b_lt_m( int m, int a, int b )
{ return sum_multiples_of_n_lt_m(m,a) + sum_multiples_of_n_lt_m(m,b) - sum_multiples_of_n_lt_m( m, lcm(a,b) ) ; }
...
int main()
{
    ...
    std::cout << sizeof( char[ sum_multiples_of_a_or_b_lt_m( 5000, 21, 35 ) ] )  << '\n' ;

    // sum of all the multiples of 33 or 77 below 8000.
    std::cout << sum_multiples_of_a_or_b_lt_m( 8000, 33, 77 ) << '\n' ;
}
