    ...
    struct compiler
    {
        ...
        void operator()(ast::operation const& x) const
        {
            ...
            switch (x.operator_)
            {
				...
				case '^': code.push_back(op_exp); break; //ADDED
    ...
    struct calculator : qi::grammar<Iterator, ast::expression(), ascii::space_type>
    {
        calculator() : calculator::base_type(expression)
        {
            ...
            BOOST_SPIRIT_DEBUG_NODES(
                (expression)(term)(unary)(power)(power_expr)(primary)); //MODIFIED
		...
		qi::rule<Iterator, ast::expression(), ascii::space_type> power_expr; //ADDED
        qi::rule<Iterator, ast::operand(), ascii::space_type> primary,unary,power; //MODIFIED
    };
}
...
main()
{
    ...
    std::istringstream iss("-2^-(-1-1)^3\nq\n"); //ADDED

    std::string str;
    while (std::getline(iss, str)) //MODIFIED
    {
