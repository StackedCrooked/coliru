...
private:
    ...
    {       
        ...
        virtual void get_snapshot(Wrapper result) override final
        {
            using SnapshotType = typename T::Snapshot;
            
            // Fast type check
            if (result.mTypeIndex != typeid(SnapshotType))
            {
                throw std::bad_cast();
            }
            *static_cast<SnapshotType*>(result.mPtr) = t.get_snapshot();
        }
