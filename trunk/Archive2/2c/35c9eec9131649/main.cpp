...
int main()
{
    ...
    for(int i=0;i<nrows;++i)    
    {
        ...
        {
            backwardIteration({i,j,l},m);
            l*=m[i][j];
        }
    }

    std::array<int, 3> rightBottomState = { nrows - 1, ncols - 1, m[nrows - 1][ncols - 1] };

	std::cout << "Minimum number of zeros: " << backwardIteration(rightBottomState, m) << "\n" << std::endl;
}
