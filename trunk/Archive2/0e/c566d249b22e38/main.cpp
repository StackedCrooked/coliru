...
Cont<T, U> swrap(F f)
{
  ...
  return Cont<T, U>{op, sync_tag{}};
}

template <typename T, typename U, typename F>
Cont<T, U> awrap(F f)
{
  auto op = [=](expected_t<T> v, callback_t<U> cb) {
    if (result(v)) f(value(v), cb);
    else cb(v);
  };
  return Cont<T, U>{op, async_tag{}};
}
...
{
  {
    auto op = awrap<int, int>(async_a)
             (swrap<int, int>(sync_a))
             (awrap<int, int>(async_a))
             (swrap<int, int>(sync_b))
             (awrap<int, int>(async_a))
             (awrap<int, int>(async_b))
