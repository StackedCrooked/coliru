...
public:
	template<typename S>
    ScorerDecorator(S scorer): XYZScorer(scorer.material_rad_len), scorer(scorer){;}
...
public:
	...
	template<typename S>
    FixZToFirstInteractionScorer(S scorer): ScorerDecorator<Scorer>(scorer){}
...
public:
	template<typename S>
    ApplyEnergyCutsScorer(S scorer):
...
int main ()
{
	...
	std::shared_ptr<XYZScorer> ptr3 =
		share(FixZToFirstInteractionScorer<ApplyEnergyCutsScorer<SomeScorer>>(scorer));
	...
	static_assert(std::is_same<
		decltype(decorate<FixZToFirstInteractionScorer, ApplyEnergyCutsScorer>(scorer)),
		decltype(share(FixZToFirstInteractionScorer<ApplyEnergyCutsScorer<SomeScorer>>(scorer)))
	>::value, "");
}
