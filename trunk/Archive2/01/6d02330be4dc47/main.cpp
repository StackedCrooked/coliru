...
{    
    ...
    std::vector<SLDBConfigParams> parameterInfo = {
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(1, 'A')),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(3, 'A')),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(1, 'B', 1)),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(1, 'B', 1)),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(1, 'B', 2)),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(1, 'B', 2)),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(1, 'B', 3)),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(1, 'B', 3)),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(1, 'B', 4)),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(1, 'B', 5)),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(1, 'C', 1)),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(1, 'C', 2)),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(1, 'A', 1)),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(2, 'A')),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(2, 'B', 1)),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(2, 'B', 2)),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(2, 'B', 3)),
        SLDBConfigParams("drname._dr", "nic1", PriorityLevel(2, 'B', 2))
    };

    // ignore the filename in the priority ordering - maybe add back later
    // as the last field to compare - not sure yet.
    const auto priorityPred = [](const SLDBConfigParams& lhs, const SLDBConfigParams& rhs) -> bool {
        ...
        return std::tie(channelLHS, priorityCharLHS, seqNumLHS) <
            std::tie(channelRHS, priorityCharRHS, seqNumRHS);
    };
    ...
    std::cout << "parameters - ordered by channel & then priority" << std::endl;
    ...
    const auto channelPred = [](const SLDBConfigParams& lhs, const SLDBConfigParams& rhs) {
        return rhs.getPriorityLevel().mChannel < rhs.getPriorityLevel().mChannel;
    };
    ...
    for (auto iter = parameterInfo.begin(); iter != parameterInfo.end();) {
        ...
        std::copy(channelRange.first, channelRange.second, std::ostream_iterator<SLDBConfigParams>(std::cout, "\n"));
        std::cout << std::endl;
#endif
        ...
        std::cout << "channel [" << iter->getPriorityLevel().mChannel << "] priorities - ordered" << std::endl;
        std::copy(ch_ptys.begin(), ch_ptys.end(), std::ostream_iterator<SLDBConfigParams>(std::cout, "\n"));
        std::cout << std::endl;
        ...
        for (auto iter1 = ch_ptys.begin(); iter1 != ch_ptys.end();) {
            ...
            std::copy(range.first, range.second,std::ostream_iterator<SLDBConfigParams>(std::cout, "\n"));
#endif                    
            std::cout << std::endl;
            // build the tree here
            if (!priorityTree) {
        ...
        channelPriorityInfo[iter->getPriorityLevel().mChannel] = priorityTreeRoot;
