...
template <typaname H>
struct Solution {
    typedef typename Solve<H, 1, 2, 3>::result result;
};
...
template <>
struct Solve<H, 1, 2, 3> {
    static const int size = SizeOf<H::Stack1>::value;
    typedef typename Solve<H, size - 1>, 1, 3, 2>::result part1;
    typedef typename part1::MoveDisc<1, 2>::result part2;
    typedef typename Solve<size - 1>, 
};
