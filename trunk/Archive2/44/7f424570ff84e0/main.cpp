...
namespace to_datagram {
  ...
  {
    short bodylen;
    memcpy(&bodylen, &header.data_[0], sizeof(bodylen));

    return  bodylen + fixed_header_length();
  }
  ...
  public:
    bool read_packet(const char *pp, int len, handler_t handler, logger_t logger)
    {
      ...
      while (true)
      {
        ...
        case estate::kCompleteBody:   on_body    (estate::kRecvHeader, handler);     break;
  ...
  private:
    ...
    void ChangeState(estate next)
    {
      ...
      if (next == estate::kRecvBody)
        body_.init(body_length(packet_length(header_)));
    }

    void ToErrorState(const std::string& msg)
    {
      ChangeState(estate::kError);
      err_msg_ += msg + "\n";
    }
    ...
    void on_body(estate next, handler_t handler)
    {
      auto is_success = on_packet(handler);
    ...
    bool on_packet(handler_t handler)
    {
      auto   msg = header_.data_;
      auto &body = body_.data_;
      msg.insert(msg.end(), body.begin(), body.end());

      return handler(&msg[0], msg.size());
    };
...
int main()
{
  ...
  {
    logger("called fail handler. I'll return false for test");
    return false;
  };
  auto success_handler = [&](const char* pp, std::size_t len)
  {
    logger("called success handler. I'll return true for test");
    return true;
  };
...

  {
    ...
    short len = 6;
    parser.read_packet((const char*)&len, sizeof(len), handler, logger);
    parser.read_packet("bbbbb", 5, handler, logger);
    parser.read_packet((const char*)&len, sizeof(len), handler, logger);
    parser.read_packet("bbbbb", 5, handler, logger);
  }

  {
    ...
    bool result = true;
    if(result == true)
      result &= parser.read_packet((const char*)&len, sizeof(len), handler, logger);
    if(result == true)
      result &= parser.read_packet("bbbbbb", 6, handler, logger);
    if(result == true)
      result &= parser.read_packet("bbbbbb", 6, handler, logger);
  }
