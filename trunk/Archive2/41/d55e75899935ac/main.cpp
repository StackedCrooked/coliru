...
int main() {
...

    {
        ...
        ba::accumulator_set<
                double, 
                ba::stats<ba::tag::mean, ba::tag::max, ba::tag::min>
        ...
        srand(42);
        auto const queries = generate_test_queries(10000);
        ...
        using hrc = std::chrono::high_resolution_clock;
        {
            ...
            std::cout << queries.size() << " 'random' OLD lookups resulted in " << callbacks << " callbacks in " << std::chrono::duration_cast<std::chrono::milliseconds>((hrc::now()-start)).count() << "ms\n";
        }

        {
            ...
            std::cout << queries.size() << " 'random' NEW lookups resulted in " << callbacks << " callbacks in " << std::chrono::duration_cast<std::chrono::milliseconds>((hrc::now()-start)).count() << "ms\n";
        }
    }
