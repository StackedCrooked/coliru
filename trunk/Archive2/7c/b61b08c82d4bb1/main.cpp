...
int main()
{
    ...
    for (size_t case_number = 1; case_number<= n_cases; ++case_number)
    {
         ...
         *
         *  ink = sum(k=0..n, pi(r+2k+1)^2-pi(r+2k)^2)
         *
         * Wolfram alpha gives the equivalence "sum pi*(r+2k+1)^2-pi*(r+2k)^2, k=0 to n"
         *
         *  ink = pi(n+1)(2n+2r+1)
         *
         * So we can solve for n: "solve pi*(n+1)*(2*n+2*r+1) = a for n" (a is "ink")
         *
         ...
         */
        double n = (sqrt(M_PI)*sqrt(8.*ink+4.*M_PI*r*r-4.*M_PI*r+M_PI)-2.*M_PI*r-3.*M_PI)/(4.*M_PI);
