...
struct LoadableFile {
    LoadableFile(const std::string& rName) : mFileName(rName)
    {}
    
    inline bool operator<(const LoadableFile& rhs) const { return mFileName < rhs.mFileName; }
    
    inline bool operator==(const LoadableFile& rhs) const { return mFileName == rhs.mFileName; }
    
    inline friend std::ostream& operator<<(std::ostream& os, const LoadableFile& rhs) {
        return os << "FileName = " << rhs.mFileName;
    }
...
public:
    ...
    inline bool operator==(const IndexContainer& rhs) const {
        if (this != &rhs) {
            return mFileInfo == rhs.mFileInfo;
        } else {
            return true;
        }
    }
