...
int main()
{
    ...
    for (auto iter = parameterInfo.begin(); 
        iter != parameterInfo.end();){
        // get the next range or identical channel parameters
        auto chRange = std::equal_range(
            iter, parameterInfo.end(), *iter, gChannelPred);
        std::vector<SLDBConfigParams> chParams(
            chRange.first, chRange.second);

        // order the parameters in (highest to lowest priority).
        std::sort(chParams.begin(), chParams.end(), std::greater<SLDBConfigParams>());
        ...
        std::vector<SLDBConfigParams> deferred;
        for (auto iter1 = chParams.begin(); iter1 != chParams.end();) {
            ...
            const auto& range = std::equal_range(iter1, chParams.end(),
                *iter1, std::greater<SLDBConfigParams>());
            std::copy(range.first, range.second,
                std::ostream_iterator<SLDBConfigParams>(std::cout, "\n"));
            std::cout << std::endl;
            std::vector<SLDBConfigParams> concurrent = std::move(deferred);
            ...
            while (defIter != range.second) {
                ...
                if (tempIter != range.second) {
                    ...
                    // point to the conflicting thread parameters
                    std::cout << "deferring..." << *(tempIter + 1) << std::endl;
                    deferred.push_back(*(tempIter + 1));
                    defIter = tempIter + 1;
                } else {
                    std::copy(defIter, tempIter, std::back_inserter(concurrent));
                    defIter = tempIter;
                }
            }
            // build the tree here
