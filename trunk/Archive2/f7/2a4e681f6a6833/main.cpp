...
/// Select coordinate change based on random direction
/// such that distance from origin is not less then current distance from origin
position next_away_position( position p, position origin )
{
    // transform coordinate system such that origin is (0,0)
    p.first -= origin.first ;
...
int main()
{
    ...
    for( std::size_t i = 0 ; i < sizeof(test) / sizeof( test[0] ) ; ++i )
    {
        ...
        std::cout << '[' << p.first << ',' << p.second << "] (final)\n\n" ;
    }
}
