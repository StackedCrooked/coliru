...
template <typename Dist, typename Engine> struct Baz {
    mutable Dist dist_;
    mutable Engine engine_;

    Baz(const Dist &dist, const Engine &engine) : dist_(dist), engine_(engine) {}

    template <typename Object> void operator()(Object &operand) const {
        std::cout << operand << std::endl;
        operand += dist_(engine_);
    }
};
...
test_map createFunMap() {
    return test_map { 
        { "bar", Bar<int32_t>(1)  },
        { "dist", Baz<std::normal_distribution<double>, std::mt19937>(std::normal_distribution<double>(0.0, 3.0), std::mt19937(10)) },
    };
}
