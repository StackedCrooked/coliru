...
//Derived.h
struct ModelLibraryG4PCS: public ModelLibrary
{
    int pair_width_=42, dipole_width_=4242, voxel_size_=999;
...
{
    ModelLibraryG4PCS d;
    d.models_.emplace("one", nullptr);
    d.models_.emplace("two", nullptr);
    d.saveHashTable();
    if (!d.loadHashTable()) {
