...
struct DataView : std::vector<std::reference_wrapper<const Data>>
{
    typedef std::vector<std::reference_wrapper<const Data>> base;

    template <typename InputIterator>
    DataView(InputIterator first, InputIterator last)
        : base(first, last)
    {
    }

    template <typename InputIterator>
    DataView(const std::pair<InputIterator, InputIterator> p)
...
public:
  ...
  void deepTest() {
    ...
    std::cout << "Running " << __FUNCTION__ << std::endl << std::endl;
    {
      ...
      std::cout << boost::size(data) << " entities retrieved." << std::endl;
      std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(
...
private:
  ...
  DataView getData(int intData01, int intData02,
                    const std::wstring &strData03) const {
    return DataView(m_data.get<byKey>().equal_range(
