...
std::ostream& operator<<(std::ostream& out, const std::type_info& ti) {
    std::unique_ptr<char, void(*)(void*)> result(abi::__cxa_demangle(ti.name(), nullptr, nullptr, nullptr), std::free);
    if(!result)
        return out << "(unknown type name: " << ti.name() << ')';
...
{   
    ...
    std::cout << typeid(decltype(add_x(a))) << std::endl; // should be Y<Z<X>>
    std::cout << typeid(decltype(add_x(b))) << std::endl; // should remain Z<Z<Y<Z<X>>>>
}   
  return 0;
}