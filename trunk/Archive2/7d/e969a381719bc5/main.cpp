...
namespace operators {
    template<class D>struct is_operator {};

    template<class T, char c, class O> struct half_apply { T&& lhs; };
    // change '*' and operator type to support other "operator delimiters":
    template<class Lhs, class Op>
    half_apply<Lhs, '*', Op> operator*( Lhs&& lhs, is_operator<Op> ) {
...
struct then_tag {};
static operators::is_operator<then_tag> then;
...
auto invoke( Tup&& tup, then_tag, F&& f )
...
{
    7
    *then* [](int x){ std::cout << x << "\n"; return std::make_tuple(x-0.5, x+0.5); }
    *then* [](auto&&){ std::cout << "boo" << "\n"; };
