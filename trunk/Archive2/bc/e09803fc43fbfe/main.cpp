...
int main()
{
    ...
    assert(parsed == clone);
        ...
        void operator()(qd_json::value const& v) const              { boost::apply_visitor(*this, v); }
        void operator()(double const& v) const                      { std::cout << v; }
        void operator()(qd_json::text const& v) const               { std::cout << v; }
        void operator()(qd_json::object::value_type const& v) const { std::cout << "\n" << v.first << ": "; (*this)(v.second); }
        void operator()(qd_json::object const& v) const             { for(auto& m: v) (*this)(m); }
