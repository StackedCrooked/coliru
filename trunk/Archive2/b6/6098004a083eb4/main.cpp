...
double simpson_method(Function_ptr f, double a, double b)
{
    ...
    for (unsigned i = 0; i < intervals_count; ++i) {
        const double interval_middle = (interval_left + interval_right) / 2;
        result += dx / 6 * (f(interval_left) + 4*f(interval_middle) + f(interval_right));
        interval_left = interval_right;
        interval_right += dx;
    }
    return result;
...
int main()
{
    ...
    const double a = 0;
    const double b = 3;
    cout << "Trapeze method: " << trapeze_method(f, a, b) << '\n';
    cout << "Rectangle method: " << rectangle_method(f, a, b) << '\n';
    cout << "Simpson method: " << simpson_method(f, a, b) << '\n';
    cout << "Correct answer: " << integral_f(b) - integral_f(a) << '\n';
}