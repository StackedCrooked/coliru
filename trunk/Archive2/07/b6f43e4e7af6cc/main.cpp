...
public:
    ...
    {
        m_handlers.emplace(std::string(typeid(Callable).name()) + std::to_string(reinterpret_cast<uintptr_t>(&f)), f);
        std::cout << "operator+=(" << typeid(Callable).name() << reinterpret_cast<uintptr_t>(&f) << ")\n";
        return *this;
    }
