...
int main()
{    
    auto thunk1 = THUNK(void(int&, bool&&), &foo);
    int x = 42;
    thunk1(x, true);
    
    auto thunk2 = THUNK(void(int&, double&&), &X::bar);
    X obj{1337};
    thunk2(&obj, x, 1729);
    
    auto itable = std::make_tuple(THUNK(void(int&, bool&&), &foo),
                                  THUNK(void(int&, double&&), &X::bar));
