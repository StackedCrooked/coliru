...
//    auto variable;  //compile error - no way to find out a type;
    ...
    auto count = std::count_if(begin(myMap), end(myMap), 
                                     [](const std::pair<const int, B>& item) { return item.second.name.empty(); });
    ...
    int x =8;
    //lambda style - tightly together - preferred.
    auto itLambda22 = std::find_if(begin(myMap), end(myMap), 
                           [&x](std::pair<const int, B>& item) { item.second.name+="haha"; return item.second.name.length() > x; });
                           
    if (itLambda22 != end(myMap))
    {
        print(*itLambda22);
    }
