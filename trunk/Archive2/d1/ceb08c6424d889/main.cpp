...
struct Packet
{
    ...
    Packet(Packet&& rhs)
    {
        rhs.base().move_construct(storage());
    }
...
private:
    ...
    {
        Impl(Alloc alloc, const char* b, const char* e) : Alloc(alloc), Base(allocate(e - b), e - b)
        { memcpy(mData, b, size()); }

        Impl(Alloc alloc, uint16_t inSize) : Alloc(alloc), Base(allocate(inSize), inSize) { }

        Impl(const Impl& rhs) : Alloc(rhs), Base(allocate(rhs.mSize), rhs.mSize)
        { memcpy(mData, rhs.mData, rhs.mSize); }

        Impl(Impl&& rhs) : Alloc(std::move(rhs.getAlloc())), Base(rhs.mData, rhs.mSize)
...
int main()
{
    ...
    for (int i = 0; i != 10; ++i)
    {
        ...
        Packet packet1(std::allocator_arg, alloc, &*data.begin(), &*data.end());
    
        std::reverse(data.begin(), data.end());
    
        Packet packet2(std::allocator_arg, alloc, &*data.begin(), &*data.end());
    
        std::cout << "packet1=" << packet1 << std::endl;
        std::cout << "packet2=" << packet2 << std::endl;
    
        std::reverse(packet2.begin(), packet2.end());
    
        std::cout << "packet1.data=" << packet1 << std::endl;
        std::cout << "packet1.size=" << packet1.size() << std::endl;
        
        std::cout << "packet2.data=" << packet2 << std::endl;    
        std::cout << "packet2.size=" << packet2.size() << std::endl;
        ...
        assert(packet1 != packet2);
        std::cout << packet1.size() << '-' << packet2.size() << std::endl << std::endl;
    }
}
