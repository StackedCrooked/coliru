...
{
    std::vector<int> all_speeds() const { return { T::speed() } ; }
    void all_speeds( std::vector<int> s ) { if( !s.empty() ) T::speed( s.back() ) ; }
};

template < typename FIRST, typename... REST > struct speeds<FIRST,REST...> : speeds<FIRST>, speeds<REST...>
{
    std::vector<int> all_speeds() const
    {
    ...
    void all_speeds( std::vector<int> s )
    {
        if( !s.empty() )
        {
...
int main()
{
    ...
    std::cout << sizeof(human) << ' ' << sizeof(cheetah) << ' ' << sizeof(bluejay) << ' ' << sizeof(trout) << '\n' ;
}
