...
struct Board {int t[6][6]; int move; bool moved; int moves[36]; 
    };

int ScoreIt(Board b, int n){
        if (b.t[0][0]==n) {return 1; } else {return 0;}
}
...
void ListMoves(Board& b)
{
    int num = 0;
        ...
        b.move=num;
        //if (num==0) {b.moved=0;} else {b.moved=1;}
        b.moved=1;
...
//PrintIt(b0.at(0));
    ...
    std::for_each(b0.begin(), b0.end(), ListMoves); 
            //for (int x=0;x<36;x++) {std::cout << b0.at(0).moves[x] << " ";} std::cout<<b0.at(0).move<<"\n";
            //std::cout << b0.at(0).move << "\n";
//PrintIt(b0.at(0));
    for (int x = 0; x<(b0.at(0).move); x++) {b0.push_back(Move(b0.at(0),x));}

    //b0.push_back(Move(b0.at(0),0));
    //b0.push_back(Move(b0.at(0),1));
    //Move(b0.at(0),1);

//std::for_each(b0.begin(), b0.end(), PrintIt); 

//std::for_each(b0.begin(), b0.end(), ScoreIt);

int Score[15];
for (uint x = 0; x<(b0.size()); x++) {
    switch (ScoreIt(b0.at(x),x)) {
    case 0:{Score[x]++;}
    case 1:{Score[x]++;}
    case 2:{Score[x]++;;}
    default:{;}
...
for (int x = 0; x<15; x++) {std::cout << "Number of Level: "<<x<<"="<<Score[x]<<"\n";}
