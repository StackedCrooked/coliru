...
int totalFitness(char **population){
  ...
  void mutate(char *chrom, double mutationRate) {
    for (int i = 0; i < LENGTH; i++) {
      ...
      if (random < mutationRate) chrom[i]=='1'?chrom[i]='0':chrom[i]='1';
    }
  }
...
int main (int argc, char **argv)
{
   ...
   // print_population(population);
    ...
    int crossover_point = crossover * LENGTH;
    int gene;
    for(gene = 0; gene<LENGTH; gene++){
        if(gene<crossover_point) child[gene]=population[x][gene];
        else child[gene]=population[y][gene];            
    }
