...
using type = mytype<-1, mytype<-1, mytype<-1, int, long>, mytype<-1, char, double>>, mytype<-1, mytype<-1, int, short>, mytype<-1, float, double>>>;
...
static_assert(std::is_same<typename assign_IDs<type>::type, mytype<0, mytype<1, mytype<2, int, long>, mytype<3, char, double>>, mytype<4, mytype<5, int, short>, mytype<6, float, double>>>>{}, "");
