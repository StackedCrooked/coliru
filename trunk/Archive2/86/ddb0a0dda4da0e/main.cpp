...
struct trap
{
    ...
    auto operator()(int s, Args&&... args)
        -> decltype((get_base(s)->*t)(std::forward<Args>(args)...))
    {
        using R = decltype((get_base(s)->*t)(std::forward<Args>(args)...));
        try
        {
...
struct Foo
{
    struct Table
    {
        std::function<int(int, short, char)> fp_53;
        std::function<int(int, short, char, double)> fp_54;
    }* table = new Table;
