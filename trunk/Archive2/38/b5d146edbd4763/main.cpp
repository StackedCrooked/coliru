...
// All C++ programs always begin execution starting at main()!
int main(void)
{
    int sizediff{0}; // Always good developer form to immediately initialize a variable.
    
    //*** EDIT *** 
    // What if it was the FIRST vector v1 that was shorter, and v2 was
    //  longer instead? (Plz try that inside the original code linked above.)
    // >OH NOES!! :O
    vector<int> v1{20, 30, 90, 2, 42};       // First vector v1 is shorter in length (5).
    vector<int> v2{1, 2, -3, 4, 5, 6, 7, 0}; // Second vector v2 is longer in length (8).
    ...
    // So, let's make this code block into a second if() test instance here instead.
    // Second case: v2 is bigger.    
    else 
    if(s2 > s1) { // This test statement checks if the size of vector 2 is greater than the size of vector 1.
        //*** EDIT *** // Let's swap the subtraction around, so we don't wind up getting a negative number.
        sizediff = s2 - s1; //sizediff will now be the difference between the second and first vector sizes
