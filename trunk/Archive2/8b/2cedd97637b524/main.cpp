...
// This is an example where ADL does not work.
std::ostream& operator<<(std::ostream& os, const std::vector<uint8_t>& buffer)
{
    for (auto& b : buffer)
    {
        if (&b != &buffer[0])
...
int main()
{
    ...
    // this works because Byte is a user-defined type
    std::cout << buf << std::endl;
    
    // proof for above statement, this does not compile:
      // std::vector<uint8_t> test_no_adl = { 0x10, 0x11 };
      // std::cout << test_no_adl << std::endl;
