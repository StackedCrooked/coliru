...
int main()
{
    ...
    {
        std::partial_sort(
                ropes.begin(), ropes.begin()+1, ropes.end(),
                [=](LRopeData const& a, LRopeData const& b) {
                return a.distanceToPoint(X,Y) < a.distanceToPoint(X,Y);
                });

        LRopeData const& closest = *ropes.begin();

        std::cout 
            << "Closest rope is at " << closest.distanceToPoint(X,Y) << ", "
            << "X:" << closest.X << " Y1:" << closest.Y1 << " Y2:" << closest.Y2 << "\n";
    }
}
