...
template <typename T>
struct tuple_indices : public indices<std::tuple_size<typename std::remove_cv< typename std::remove_reference<T>::type>::type>::value> { };
...
auto awesome_tuple_dog( stuff<indices_list<I...>, indices_list<J...>, Indices...>, Tuple0&& t0, Tuple1&& t1, Tuplen&&... tn ) {
    return awesome_tuple_dog( stuff<indices_list<J..., (I + sizeof...(J))...>, Indices...>(), std::make_tuple( std::get<I>( std::forward<Tuple0>( t0 ) )..., std::get<J>( std::forward<Tuple1>( t1 ) )... ), std::forward<Tuplen>( tn )...  );
}
