...
public:
  ...
  template <typename U>
  auto bind(U&& u) const -> decltype(u(std::declval<ValueType>()))
  {
    static_assert(IsExpected<decltype(u(std::declval<ValueType>()))>::value, "u must return expected_t");
    if (valid())
      return u(*value_);
    else
      return unexpected();
...
auto bind(U&& u, F&& f) -> decltype(f(std::declval<typename U::ValueType>()))
{
  return u.bind(f);
}
...
////////////////////////////////////////////////////////////////////////////////////////
// use expected plus bind
////////////////////////////////////////////////////////////////////////////////////////
namespace use_expected_plus_bind {
...
auth_asset::Expected<std::string> func(std::string input)
{
    using auth_asset::bind;
    return bind(parse_int(input),      [&] (int i1) {
    return bind(convert_to_string(i1), [&] (std::string s1) {
    return bind(parse_int(s1),         [&] (int i2) {
    return bind(convert_to_string(i2), [&] (std::string s2) {
      return auth_asset::Expected<std::string>(s2);
    });});});});
}

void run()
{
    std::cout << "= use_expected_plus_bind ==================" << std::endl;
    auto ret1 = func("123");
    if (!ret1.valid())
        std::cout << "error. " << ret1.error() << std::endl;
    else
