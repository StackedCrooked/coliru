...
struct reference_collector
{
    ...
    {
        int dummy[] = {(v.emplace_back(std::forward<typename std::tuple_element<Is, decltype(m)>::type>(std::get<Is>(m))), 0)...};
        (void)dummy;
    }
...
int main()
{
    ...
    std::vector<TS> vec( collect_references(ts1, std::move(ts2)) );
}
