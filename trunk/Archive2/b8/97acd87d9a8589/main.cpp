...
{
	template<typename R>
	auto operator*(boost::future<R> &&ft) -> decltype(ft.get())
	{
		//int zz = ft;
		...
		auto result = ft.then(boost::launch::async, [current_coro](boost::future<R> ft) -> Result
		{
...
void async_user_handler()
{
	...
	for (auto & f : fs)
		cout << await std::move(f) << ":\tafter end" << endl;
}
