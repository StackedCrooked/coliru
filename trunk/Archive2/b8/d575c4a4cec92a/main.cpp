...
struct Data {
    ...
    using Folders = std::map<Id, Folder>;
 ...
 private:
    static std::pair<Id, Folder> extract_folder(qd_json::object const& obj) {
        return { 
            id(obj.at("id")),
            {
                id   (obj.at("id")),
    ...
    static std::pair<Id, File> extract_file(qd_json::object const& obj) {
        return { 
            id(obj.at("id")),
            {
                id   (obj.at("id")),
...
int main() {
    ...
    using namespace boost::adaptors;
    for (auto& e : extracted.folders | map_values) 
        std::cout << "folder:\t" << e.id << "\t" << e.path << "\n";
    for (auto& e : extracted.files   | map_values) s
        td::cout << "file:\t"   << e.id << "\t" << e.path << "\t" << e.name << "\n";
}
