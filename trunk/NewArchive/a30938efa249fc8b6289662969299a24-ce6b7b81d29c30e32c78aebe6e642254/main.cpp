     ...
     std::tuple<T, T, T> properties(const Collection<T, TAlloc>& coll) 
{
     ...
     auto f = std::minmax_element(coll.begin(), coll.end());
     std::get<reduction::min>(result) = *(f.first);
     std::get<reduction::max>(result) = *(f.second);

     T initVal(0);
     T sum = std::accumulate(coll.begin(), coll.end(), initVal);
     std::get<reduction::sum>(result) = sum;
