...
template<class T, class hash=std::hash<T>, class equal=std::equal_to<T>, class allocator=std::allocator<T>>
...
public:
    ...
    std::unordered_map<T, std::function<void()>, hash, equal, allocator> cases;
