...
#define ENABLE_CHECK_TYPEDEF(Name)  \
    ...
    \
    template<class T> \
    struct BOOST_PP_CAT(HasTypedef_, Name)<T, typename std::common_type<typename T::Name>::type> \
    { \
        enum { value = 1  }; \
    };
    
    
#define HAS_TYPEDEF(Class, Name) \
    static_cast<bool>(BOOST_PP_CAT(HasTypedef_, Name)<Class>::value)
