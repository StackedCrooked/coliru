...
struct MyStruct
 {
     ...
     void set_mXa(signed v) {bits&=0xFFF8; bits|=(v&0x7);}
     unsigned get_mXb() const {return bits&0x7;}
     void set_mXb(unsigned v) {bits&=0xFFF8; bits|=(v&0x7);}
     
     signed get_mYa() const {return (bits>>3)&0x7;}
     void set_mYa(signed v) {bits&=0xFFC7; bits|=((v&0x7)<<3);}
     unsigned get_mYb() const {return (bits>>3)&0x7;}
     void set_mYb(unsigned v) {bits&=0xFFC7; bits|=((v&0x7)<<3);}
     
     MyEnum get_mEnum() const {return MyEnum((bits>>6)&0x3);}
     void set_mEnum(MyEnum v) {bits&=0xFF3F; bits|=((v&0x3)<<6);}
