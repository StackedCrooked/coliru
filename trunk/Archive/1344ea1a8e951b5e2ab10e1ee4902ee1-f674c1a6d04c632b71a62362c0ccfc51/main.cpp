...
const string upperInput("01101101111000101010");
...
int main()
{
    ...
    // a state is isolated if the state before and after holds the opposite value (or is begin/end)
    const auto isIsolatedState = [] (const vector<bool>& source, const vector<bool>::iterator& it) {
        return (it == source.begin() || *(it - 1) != *it)
            && (it == (source.end() - 1) || *(it + 1) != *it);
    };
