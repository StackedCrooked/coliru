/*
 * I want to insert a small DSL into a server application I work on. The syntax
 * is very simple and even at this early stage I am stumped. I just can't get
 * my head around how to construct the syntax in spirit.
 * 
 * Here is an example of the syntax I want to test for:
 * 
 *      WHERE [not] <condition> [ and | or <condition> ] <command> [parameters]
 * 
 * The WHERE clause will select a number of objects from an internal store by
 * testing named properties on them. The vector of selected objects is then
 * passed as input to the command object.
 * 
 * There are 2 posible tests I want to perform on each object:
 *
 *      <property> = "value"
 * 
 * and
 ...
 * 
 * I know this is a big ask, but I am wondering if there are any spirit experts
 * out there who can knock out this syntax in a few seconds flat? I got as far
 * as parsing LIKE and =, but got stuck when trying to mix that with AND, OR
 * and NOT. The problem for me is not knowing where to start when thinking
 * about how spirit will tackle this.
 */
    ...
    struct parser : qi::grammar<It, statement(), Skipper>
{
    parser() : parser::base_type(start)
    {
        ...
        BOOST_SPIRIT_DEBUG_NODE(condition_); BOOST_SPIRIT_DEBUG_NODE(value_); BOOST_SPIRIT_DEBUG_NODE(strlit_); BOOST_SPIRIT_DEBUG_NODE(regex_); BOOST_SPIRIT_DEBUG_NODE(property_);
        BOOST_SPIRIT_DEBUG_NODE(statement_); BOOST_SPIRIT_DEBUG_NODE(filters_); BOOST_SPIRIT_DEBUG_NODE(print_); BOOST_SPIRIT_DEBUG_NODE(set_); BOOST_SPIRIT_DEBUG_NODE(command_);
    }
