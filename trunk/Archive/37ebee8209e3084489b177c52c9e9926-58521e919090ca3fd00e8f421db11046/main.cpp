...
int main(int argc, char** argv)
{
    ...
    constexpr auto Mode = std::consume_header;
    file.imbue({ file.getloc(), new std::codecvt_utf16<char32_t, 0x10ffff, Mode> });
    // File exposes UCS4 code units from e.g. UTF-16LE physical bytes

    std::stringstream arena;

    // Fill arena with UTF-8 bytes, from UCS4 code units
    {
    ...
    // Expose UTF-16 code units from UTF-8 bytes
    std::wbuffer_convert<std::codecvt_utf8_utf16<char16_t>, char16_t> from_bytes { arena.rdbuf() };
