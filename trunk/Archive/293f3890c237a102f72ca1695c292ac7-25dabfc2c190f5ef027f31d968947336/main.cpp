...
struct avg
{
    template<
        ...
        class Index = typename std::iterator_traits<RandomIt1>::value_type, class Rank = double 
    >
...
struct max
{
    ...
    void operator()(RandomIt1 first, RandomIt1 last, RandomIt2 dst) const 
    {
        auto const m = *std::max_element(first, last, [&](Index const& L, Index const& R) {
            return dst[L] < dst[R];    
        });
        for (auto it = first; it != last; ++it)
            dst[*it] = m;
...
struct min
{
    ...
    void operator()(RandomIt1 first, RandomIt1 last, RandomIt2 dst) const 
    {
        auto const m = *std::min_element(first, last, [&](Index const& L, Index const& R) {
            return dst[L] < dst[R];    
        });
        for (auto it = first; it != last; ++it)
            dst[*it] = m;
...
void rank_n(RandomIt1 src, Size n, RandomIt2 ord, RandomIt3 dst, Compare cmp = Compare())
{
    ...
    for (auto it = ord; it != ord + n; /* increment inside loop */) {
        auto const tied = std::equal_range(ord, ord + n, *it, [&](Index2 const& L, Index2 const& R){
            return cmp(src[L], src[R]);    
        });
        auto const dist = std::distance(tied.first, tied.second);
        if (dist != 1)
            TieBreak()(tied.first, tied.second, dst);
        std::advance(it, dist);
    }
}
