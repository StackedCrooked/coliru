...
struct Car
{
    ...
    unsigned weight() const { return 20; }
};

int main()
{
    Actor<Car> car;
    std::cout << car.execute<unsigned>([](Car & c) { return c.age(); }).get() << std::endl;
    std::cout << car.execute<unsigned>([](Car & c) { return c.weight(); }).get() << std::endl;
}