...
auto make_tstruct(Attr& attr, const std::string& name, const Parser& p) -> decltype(boost::proto::deep_copy(qi::lit(name) >> p[ ph::ref(attr) = qi::_1 ]))
{
    return boost::proto::deep_copy(qi::lit(name) >> p[ ph::ref(attr) = qi::_1 ]);
}

struct join
{
	template <typename State, typename Parser>
	auto operator()(const State& state, const Parser& parser) const -> decltype (boost::proto::deep_copy(parser^state))
	{
		return boost::proto::deep_copy(parser^state);
	}
};
