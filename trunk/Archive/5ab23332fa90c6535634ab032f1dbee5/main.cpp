...
int main() {
    {
        ...
        int array[3][4] = {{0, 2, 5, 8},
                           {1, 4, 7,10}, 
                           {3, 6, 9,11}};    
        auto last = make_diag(array, 3, 4, false);
        for (auto first=make_diag(array, 3, 4, true); first!=last && giveup-->0; ++first) 
            std::cout << *first << '\n';
    }
    std::cout << '\n';
    {
        ...
        int array[4][3] = {{0, 2, 5},
                           ...
                           {3, 7,10},
                           {6, 9,11}};    
        auto last = make_diag(array, 4, 3, false);
        for (auto first=make_diag(array, 4, 3, true); first!=last && giveup-->0; ++first) 
            std::cout << *first << '\n';
    }
    return 0;
}
