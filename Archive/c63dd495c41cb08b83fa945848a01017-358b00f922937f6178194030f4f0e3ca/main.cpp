...
__asm__ (R"foo(
.globl twos_complement_checksum
.type twos_complement_checksum,@function
.align 16
...
12: mov %r8w, %ax
    not %ax
    retq
.size twos_complement_checksum, .-twos_complement_checksum
)foo");


extern "C" unsigned short twos_complement_checksum(const unsigned char*, int);


int main()
{
    const unsigned char data[] = { 0x01, 0x02, 0x03, 0x04, 0x05 };
    std::cout << twos_complement_checksum(data, sizeof(data));
}
