...
//	bool value2 = true;
	...
	if(!value1)
		std::cout <<"Error while loading global default database"<<std::endl;
...
//	std::cout << "Pushed param2" << std::endl;
	...
	if(!value1)
		std::cout <<"Exception should have been raised when "
				"pushing a database of the same id than the previous one!"<< std::endl;
	...
	if(!value1)
		std::cout <<"Exception should have been raised when pushing a global default "
				"database when one has already been pushed for the same!"<< std::endl;

	db_param2->setManufacturer(DatabaseManufacturer::SQLITE3);
	value1 = false;

	try {
		value1 = DatabaseHandler::pushDatabaseParameters(db_param2, false, true);
	} catch(DatabaseParametersLoadingException &e) {
		ss << "No exception should have been raised: " << e.what();
	...
	try {
		value1 = DatabaseHandler::pushDatabaseParameters(db_param3);
	} catch(DatabaseParametersLoadingException &e) {
		ss << "No exception should have been raised: " << e.what();
        std::cout << ss.str() << std::endl;
		ss.str("");
