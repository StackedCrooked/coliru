...
struct max
{
    ...
    void operator()(RandomIt first, RandomIt last) const 
    {
        auto const m = std::max(first, last);
...
struct min
{
    template<class RandomIt, class Index = typename std::iterator_traits<RandomIt>::value_type >
    void operator()(RandomIt first, RandomIt last) const 
    {
        auto const m = std::min_element(first, last);
        std::transform(first, last, first, [&](Index const&){ return m; }); 
    }
};

struct avg
{
    template<class RandomIt, class Index = double >
    void operator()(RandomIt first, RandomIt last) const 
    {
        auto const m = static_cast<double>(std::accumulate(first, last, 0) / std::distance(first, last));
        std::transform(first, last, first, [&](Index const&){ return m; }); 
    }
};
...
template<
    class RandomIt1, class Size, class RandomIt2, class RandomIt3, class TieBreak = tie_break::max,
    class Compare = std::less< typename std::iterator_traits<RandomIt1>::value_type >,
    class Index2 = typename std::iterator_traits<RandomIt2>::value_type
>
void rank_n(RandomIt1 src, Size n, RandomIt2 ord, RandomIt3 dst, Compare cmp = Compare())
{
    ...
    for (auto it = ord; it != ord + n; ) {
        ...
        it += std::distance(tied.first, tied.second);
    }
}
...
int main() {
    ...
    auto ord = std::vector<double> {};
    ord.reserve(N);
    iota_n(std::back_inserter(ord), N, 0);
    
    auto rnk = std::vector<double> {};
    rnk.reserve(N);
    iota_n(std::back_inserter(rnk), N, 0);
