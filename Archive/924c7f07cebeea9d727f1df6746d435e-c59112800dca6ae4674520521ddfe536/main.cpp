...
template<typename C, EnableFuncIf< !has_size_method<C>::value &&
  std::is_same< std::random_access_iterator_tag, IteratorCategory<Iterator<C>> >::value, 2>... >
std::size_t size_at_least( C&& c ) {
    using std::begin; using std::end;
  return end(c)-begin(c);
};
template<typename C, EnableFuncIf< !has_size_method<C>::value &&
  !std::is_same< IteratorCategory<Iterator<C>>, std::random_access_iterator_tag >::value, 3>... >
std::size_t size_at_least( C&& c ) {
  return 0;
};
...
{
  using std::begin; using std::end;
  try_reserve( c1, c1.size() + c2.size() );
  if(std::is_rvalue_reference<C2&&>{})
  {
      c1.insert( end(c1),
                 make_move_iterator(begin(c2)), make_move_iterator(end(c2)) );
  }else
  {
