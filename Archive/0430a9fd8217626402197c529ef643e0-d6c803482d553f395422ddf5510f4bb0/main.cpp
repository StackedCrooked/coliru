...
namespace Furroflect {
	...
	public:
    	...
    	: vendor( vend ), compilerargs( argc, argv ), errorstream( errors ) {
    	...
    	bool Parse( Furrovine::String filename ) {
    		...
    		clang::DiagnosticsEngine diagnosticsengine( diagnosticids, &diagnosticsoptions, new clang::TextDiagnosticPrinter( errorstream, &diagnosticsoptions, false ), true );
    		...
    		clang::HeaderSearch hs( &headersearchoptions, filemanager, diagnosticsengine, languageoptions, targetinfo.get( ) );
    		llvm::IntrusiveRefCntPtr<clang::PreprocessorOptions> llvmpreprocessoroptions( &preprocessoroptions );
